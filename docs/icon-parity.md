...
The post-review search-results correction follows the separate pinned `Mn` caller: 18-point artwork, 6-point vertical padding around a 21-point default line box, 12-point result gaps and 4-point top padding. The 30-point/31-point measurements above apply to normal navigation rows, not search results. Only the active search option receives highlight paint; the current page is not separately painted in the result list. This corrected search composite and geometry require fresh native evidence; the preceding captures bind `23ccff3`.
