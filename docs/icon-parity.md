...
The post-review search-results correction follows the separate pinned `Mn` caller: 18-point artwork, 6-point vertical padding around a 21-point default line box, 12-point result gaps and 4-point top padding. The 30-point/31-point measurements above apply to normal navigation rows, not search results. Only the active search option receives highlight paint; the current page is not separately painted in the result list. Fresh `dbffa06` actual-App captures observe the 33-point result height, 18-point glyph, 12-point gap and coherent combobox/listbox/option accessibility state. These search states have no matched helper reference capture and do not establish pixel parity.
