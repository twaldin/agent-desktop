...
The runtime hardening currently passes30 source tests/146 assertions, with the actual systemd parser skipped on macOS, plus typecheck and independent source review. The separately verified older desktop candidate already resolves its SDK/CLI inside its bundle with zero hostile PATH sentinel invocations. That older artifact does not establish acceptance of these new startup guards; fresh artifact verification remains separate.
