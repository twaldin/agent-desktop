...
Home now runs desktop and host **`spike-2026-09-05-14`**; Work runs desktop/host 13 and Deckbox host 13. Home’s guarded upgrade preserved catalogs, drafts and original native bytes, with one checked native shutdown exit append; its pre-upgrade backup matches and data remains schema 2. Release 14 supports schemas `[1,2,3]` and IPC 4. Its archive SHA-256 is `9724250f4222b17b8b19fe3cd943072bb671dfc32d09a23818cf1e103d363052`. Image-save migration, physical remote adoption and provider-image acceptance remain pending. Evidence: `.data/ui-acceptance/home-release14-upgrade.json` and `home-release14-preservation.json`.
...
The latest physical rollback cycle remains Home's **12→11→12**, completed before saving permission intent. After its actual composer save promoted data to schema 2, an old-code rollback refused before quitting the unchanged live app. Evidence: `home-release12-rollback-to11-preservation.json`, `home-release12-return-preservation.json` and `release12-installed-rollback-refusal.json`. Release 13 keeps the same supported schemas; its successful upgrade is not a new physical rollback proof. Current UI/provider evidence, the post-restart Work-discovery issue and remaining installation gates are in [status.md](status.md).
