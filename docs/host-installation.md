...
Known targets: `twaldin@twaldin-work` uses `/opt/homebrew/bin/bun`; `tim@deckbox` uses `/home/tim/.bun/bin/bun`. Home uses `/Users/twaldin/.bun/bin/bun`. Installation copies the verified Bun executable into the release, runs `bun install --production --frozen-lockfile --backend=copyfile`, and imports the host/native lock library before activation. Dependency files are copied independently of the package cache. Bun and native binaries then remain independent of the checkout and later global Bun upgrades.

New source29 artifacts declare `apps/host/src/packaged-entry.ts`. This bootstrap validates Bun1.3.14, the five direct OMP18.1.10 packages and their OMP dependency ownership before importing the host. Workers use the adjacent packaged bootstrap and the same absolute bundled Bun. The desktop owns these under `Contents/Resources/runtime/bun` and `Contents/Resources/host`; installed hosts own them inside their immutable version directory. Missing or externally redirected runtime files fail startup. Packaged desktops ignore `AGENT_DESKTOP_BUN` and `AGENT_DESKTOP_PROJECT_ROOT`; those overrides remain available for development. No global `omp` command supplies application code. Native account/configuration/broker reuse is unchanged.

The runtime hardening passes30 source tests/146 assertions, with the actual systemd parser skipped on macOS, plus typecheck and independent source review. A separate uninstalled artifact at `out/runtime-owned-2026-09-06/desktop/Agent Desktop.app` freezes source `f3b03408fb0e8850526319f1e6f513774e5f8bb3`, host version `spike-2026-09-06-runtime-owned`, and schemas1–5. Its host archive SHA-256 is `a29923505cbb261f8c0182f80094763df4c023f35cfaeb5459e0776b569765e8`. Deep/strict signing, all143 host source hashes, and30,169 artifact file/link records pass. Read-only bundled guard/resolution probes succeed under hostile and empty PATH, with zero global executable sentinel invocations and zero shared dependency hardlinks. Packaging imports the server/native lock library without starting the service. These probes do not initialize an OMP session or establish installed behavior. The exact pair and `.data/runtime-owned-package/receipt.json` were delivered through the verified Herdr pane for independent isolated SDK/worker checks. Subsequent schema6 source changes are outside this frozen artifact.

Independent Work acceptance passes for that exact uninstalled f3b0340 pair. Both hostile and empty PATH runs used bundled Bun1.3.14/OMP18.1.10; two actual packaged discovery workers initialized and acknowledged disposal with exit0, without sessions or prompts. The compiled desktop resolver, host/worker bootstraps, SDK ownership checks, exact Bun guards and archive validators reject the tested missing, external, wrong-version and tampered cases. All30,169 entries/143 host hashes and the original deep/strict signature matched before and after; no dependency hardlinks or sentinel executions were observed. No Electron UI, installation, provider prompt, profile/auth change or existing fixture action occurred. Home verified the copied independent receipts against the supplied SHA-256 values: `verification.json` = `38f2a3bfe12528ef5dc0b07a3249ef167c9b0deeda0419478f889919f12e9888`; `verification-runs.json` = `5e35c13e5a5f9736c3d829fbdff5d747970bcaaa19ded864e97cc7fed08e6cd4`. Private evidence is `.data/runtime-owned-package/work-independent/`; the untouched Work artifact remains under `/Users/twaldin/.agent-desktop-runtime-owned.vAOKQ7/artifact`. This is runtime ownership acceptance for f3b0340/schema1–5, not schema6 or pixel acceptance.
...
All three hosts now run **`spike-2026-09-05-17`**; desktop installation status is tracked in [status.md](status.md). The archive SHA-256 is `00939e0c22187ba49abbfc4258ce74aaaa09f10848b049d9ce6d1b5f8ef2c32d`. Release 17 contains 109 hashed files, supports schemas `[1,2,3]`, and uses worker IPC 6. Frozen installs reproduce the selected OMP 18.1.10 browser metadata patch on macOS and Linux; global OMP and previous release trees remain unchanged.

The guarded 16→17 upgrades preserved catalogs, drafts, host identities and original native-history prefixes. Only verified native shutdown exit entries were appended. Backups passed `quick_check`; Home remains schema 2 and the remote hosts remain schema 1. Work and Deckbox retain verified release 16 trees. Deckbox additionally passed five isolated HTTP/native-worker tests with 18 assertions and provider access blocked. This establishes installation and metadata transport, not a browser panel or a new rollback cycle. Evidence: `.data/ui-acceptance/installed-hosts-release17.json`, `home-release17-preservation.json` and `home17-installed-contract.json`.

At the prior release 13 checkpoint, all three selected hosts ran **`spike-2026-09-05-13`** and both Macs ran desktop 13. The archive SHA-256 is `a3dc62fe445eae1095471cebf0e07d5163ac8546d127daca4cc222fd6859ed1f`. Work and Deckbox verified all 93 artifact files, bundled Bun/tmux, native imports, IPC 3 and schemas `[1,2]`; catalogs/drafts and original native prefixes survived their guarded 12→13 upgrades, and private backups matched. Both retained exact release 12 trees. Deckbox separately passed seven isolated native Auto/worker tests with 56 assertions against the exact archive sources plus explicitly copied fixtures; those fixtures were removed and its unchanged installed service reverified. Home's 12→13 upgrade and subsequent native Auto restart preserved catalogs/drafts and all original native bytes, allowing only checked shutdown `session_exit` appends. Evidence: `.data/ui-acceptance/installed-hosts-release13.json`, `home-release13-preservation.json` and `release13-auto-restart-preservation.json`.

The latest physical rollback cycle remains Home's **12→11→12**, completed before saving permission intent. After its actual composer save promoted data to schema 2, an old-code rollback refused before quitting the unchanged live app. Evidence: `home-release12-rollback-to11-preservation.json`, `home-release12-return-preservation.json` and `release12-installed-rollback-refusal.json`. Release 17 supports schemas `[1,2,3]`; its successful upgrade is not a new physical rollback proof. Current UI/provider evidence, the post-restart Work-discovery issue and remaining installation gates are in [status.md](status.md).
