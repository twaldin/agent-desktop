...
Controlled Electron checks cover large catalogs, exact provider/model selection, disabled controls, keyboard dismissal, effort changes, reset, and narrow/zoomed layouts. The first popup now uses a 256-point summary, reset and pill-shaped ordinal slider. Its model summary opens catalog search; the effort button opens all native choices, including Auto, Off and default. This effort button occupies the reference fast-mode position intentionally: a generic cross-provider fast-mode mapping has not been established. No control claims to enable Codex fast mode. Effort display capitalization does not change the native values sent to OMP.

Fresh chats keep the composer at the bottom and center the welcome content in the remaining conversation area. Source checks in `.data/power22-controls/` exercise the actual component with Electron input; `.data/power22-production-fresh/` captures the full production App with controlled transport, including bottom anchoring and an unobscured model-summary hit target at1440×1000. The first-level shape is based on refreshed8109 `02-models/02` and submenu `06`; these are structural references, not a registered pixel comparison. OMP catalog contents, independent effort access and absent unverified fast-mode mapping are explicit differences. Fonts, exact hover/animation states, slider track geometry and installed appearance remain verification work.
