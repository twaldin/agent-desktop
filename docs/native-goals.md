...
The editor uses a held base fingerprint, so a remote edit rejects rather than overwriting authored text. It exposes optional native token budget controls. Local dirty edits currently survive refresh/error while the editor remains mounted, but are not yet durable across tab close or desktop restart. The `/goal` composer-intent chip, new-conversation goal submission, durable editor drafts, and the remaining native command-menu integration still need implementation. The current application action opens the editor for an existing conversation; it is not full command parity.
