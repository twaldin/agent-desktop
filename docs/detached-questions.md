...
Independent Work21 acceptance now covers the compact card in a native window at the recorded1440×1000 frame:736×232-point card,544×28-point own-response field, native custom density/detail entry, one acceptance and one ordinary answer delivery after the exact fixture gate release. The running model remained the controlled local provider; no hosted-provider or approval claim is added. External evidence lives in the immutable refresh sibling under `94-release21/03–06` and `public-question-evidence`.
