...
The original artifact batch supports standard MCP Apps resources and an already loaded selected session’s catalogue. The subsequent [directory-owner and resource-subscription flow](mcp-apps.md#apps-without-a-conversation) adds explicitly connected project/default-directory viewers and original file-change notifications; it does not create a global configuration policy or turn historical tool results into directory-owned sources. Retained unselected-session catalogue discovery, legacy Skybridge applications, arbitrary viewer formats and full physical/native parity remain separate requirements. Files tree/path operations keep their separate owner and review scope.
