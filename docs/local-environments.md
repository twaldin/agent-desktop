...
The settings editor and sidebar are being integrated with this transport. Editor state stores per-host/project catalogs, unsaved edits and original pending save identities in the desktop cache; offline editing, stale writes and lost receipts have focused state tests. Git and environment settings routes now restore as their own pages, rather than Git reopening as OMP. The focused combined checks pass48 tests with249 assertions, and typecheck passes. Actual editor interaction/capture acceptance remains pending.
