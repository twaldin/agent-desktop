...
export function parseNativeSessionMcpSnapshot(value: unknown): NativeSessionMcpSnapshot {
  const input = record(value);
  exactKeys(input,['epoch','revision','available','reason','servers'],'Unsupported native MCP catalog field.');
  ...
  const parsed = {epoch:text(input.epoch,200),revision:integer(input.revision),available:input.available,servers,...(input.reason === undefined ? {} : {reason:text(input.reason,4096)})};
