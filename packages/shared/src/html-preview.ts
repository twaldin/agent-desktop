...
export interface HtmlPreviewLease { leaseId: string; epoch: string; entryId: string; revision: string; url: string; workerPid: number; expiresAt: number; validForMs: number }
...
export function parseHtmlPreviewLease(value: unknown, request: HtmlPreviewRequest): HtmlPreviewLease {
  ...
  if (!v || v.epoch !== request.epoch || v.entryId !== request.output.entryId || v.revision !== request.output.revision
  ...
  return { leaseId, epoch: v.epoch, entryId: v.entryId, revision: v.revision, url: url.href, workerPid: v.workerPid!, expiresAt: v.expiresAt!, validForMs: v.validForMs! };
}
