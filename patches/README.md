...
Bun 1.3.14 has a [directory-mode bug when patches create nested files](https://github.com/oven-sh/bun/pull/33573). The installer encoding creates 13 new files with mode `100755`, followed by explicit mode-only changes to their intended `100644`. This lets Bun create searchable parent directories; the installed files end with their original intended permissions. Do not remove those final mode changes or make the source files permanently executable.
