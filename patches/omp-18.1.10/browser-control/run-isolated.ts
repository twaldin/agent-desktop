...
const base = path.join(repository, ".data/temp/omp-browser-control"); await mkdir(base, { recursive: true, mode: 0o700 }); const directory = await mkdtemp(path.join(base, "candidate-check-"));
...
for (const [file, digest] of Object.entries(artifact.patchedFiles)) assert.equal(createHash("sha256").update(await readFile(path.join(packageRoot, file as string))).digest("hex"), digest);
...
const child = Bun.spawn([process.execPath, path.join(import.meta.dir, "control-contract.ts"), runRoot], { cwd: runRoot, env: { PATH: process.env.PATH, HOME: runRoot, TMPDIR: runRoot, TERM: "dumb", SHELL: "/bin/sh", BROWSER_CONTROL_NATIVE_PACKAGE: packageRoot, PUPPETEER_EXECUTABLE_PATH: await browserExecutable(), PI_BROWSER_CMUX: "0", PI_BROWSER_RELAY: "0" }, stdout: "inherit", stderr: "inherit" }); assert.equal(await child.exited, 0, `Candidate contract failed; inspect ${directory}`);
