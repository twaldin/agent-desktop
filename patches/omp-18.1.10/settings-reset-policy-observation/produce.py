#!/usr/bin/env python3
"""Regenerate the complete coding-agent patch with the authored Settings pair."""
...
parser.add_argument("--settings-src", type=Path, required=True)
parser.add_argument("--settings-dts", type=Path, required=True)
...
shutil.copyfile(args.settings_src, args.work / "src/config/settings.ts")
shutil.copyfile(args.settings_dts, args.work / "dist/types/config/settings.d.ts")
