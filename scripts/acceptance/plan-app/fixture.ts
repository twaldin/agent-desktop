...
export interface PlanFixtureContext {
...
export async function startPlanFixture(output?: string) {
  const root = await realpath(await mkdtemp(join(tmpdir(), 'agent-desktop-plan-app-')));
  ...
  const environment = { HOME: root, USER: username, LOGNAME: username, PATH: privateBin + delimiter + (process.env.PATH ?? '/usr/bin:/bin:/usr/sbin:/sbin'), TMPDIR: tmpdir(), PI_CODING_AGENT_DIR: join(root, 'agent'), PI_DISABLE_DOTENV: '1',
    AGENT_DESKTOP_NATIVE_TERMINALS: '0', AGENT_DESKTOP_DATA_DIR: join(root, 'data'), AGENT_DESKTOP_PROFILE_DIR: join(root, 'profile'), TERM: 'dumb', PLAN_APP_FIXTURE_ROOT: root };
  ...
  try {
    ...
    while (!await Bun.file(join(root, 'ready.json')).exists()) {
      if (host.exitCode !== null || Date.now() > deadline) throw new Error(`Plan fixture did not start; inspect ${logs}`);
      await Bun.sleep(50);
    }
    const { connection, context } = JSON.parse(await readFile(join(root, 'ready.json'), 'utf8')) as { connection: LocalConnection; context: PlanFixtureContext };
    if (connection.pid !== host.pid || new URL(connection.origin).hostname !== '127.0.0.1') throw new Error('The Plan fixture host identity does not match its owned child.');
    return { root, environment, connection, context,
      async stop() {
        ...
        if (code !== 0) throw new Error(`Plan fixture cleanup failed (${code}); isolated root retained: ${root}`);
        await Bun.write(join(logs, 'fixture-evidence.json'), JSON.stringify({ context, hostPid: host.pid, stopped: true }));
        // Capture the final journal bytes only after the actual worker drains.
        // Failed capture retains the private fixture, just like failed shutdown.
        if (output) {
          const capture = async (relative = ''): Promise<void> => {
            for (const entry of await readdir(join(root, 'agent', relative), { withFileTypes: true })) {
              if (entry.isSymbolicLink()) throw new Error('Unexpected symlink in native Plan evidence.');
