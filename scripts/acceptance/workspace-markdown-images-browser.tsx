...
const bridge={...(window as any).agentDesktop,workspaceQuery:(owner,query,selectedHost)=>request('/v1/workspace/query',{target:owner,query,owner:selectedHost}),command:(envelope,selectedHost)=>request('/v1/commands',{...envelope,owner:selectedHost}),subscribe:listener=>{listeners.add(listener);return()=>listeners.delete(listener)}} as Pick<DesktopBridge,"workspaceQuery"|"command"|"subscribe"|"acquireWorkspaceImage"|"releaseWorkspaceImage">;
let current:WorkspaceState,dock:ReturnType<typeof useWorkbenchDock>,setConnection:(value:boolean)=>void, reveal:(line:number,column?:number)=>void;
function Fixture(){const[connected,setConnected]=useState(true);setConnection=setConnected;const[fileRequest,setFileRequest]=useState<WorkspaceFileRequest>();const data=useMemo(()=>new WorkspaceState(bridge,hostId,target,offlineCache,hostId),[]);current=data;useEffect(()=>()=>data.stop(),[data]);dock=useWorkbenchDock(bridge as DesktopBridge,defaultWindowView(),hostId,target,connected,e=>errors.push(e));
 reveal=(line,column)=>setFileRequest({path:"docs/guide.md",line,column,id:crypto.randomUUID()});
 ...
 return <main className="workspace-markdown-images-fixture"><button onClick={()=>open('docs/guide.md')}>Open guide</button><button className="fixture-focus">Fixture focus</button><DockPanel destination="right" state={dock.snapshot.state} tabs={dock.snapshot.tabs} viewport={{width:innerWidth,height:innerHeight}} onChange={dock.change} renderTab={(tab,active)=><WorkspacePanel embedded active={active} data={data} connected={connected} filePath={tab.filePath} fileRequest={fileRequest?.path===tab.filePath?fileRequest:undefined} fileMode={tab.fileMode} onFileModeChange={mode=>dock.setFileMode(tab.id,mode)} onOpenFile={open} openExternal={async url=>{external.push(url)}} name="Images fixture" path={project} onClose={()=>{}} onOpenProject={async()=>{}}/>}/></main>;
}
