...
const documentId = crypto.randomUUID(),
  api = window.contextMaintenanceAppFixture;
...
Object.assign(window, {
  contextMaintenanceAppCall(method: string, args?: unknown[]) { return api.call(method, args); },
  contextMaintenanceAppTarget(selector: string, text?: string) {
  ...
  contextMaintenanceAppState() {
    ...
    return {
      ...
      prompt: { text: prompt?.textContent, disabled: prompt?.getAttribute("aria-disabled"), focus: document.activeElement === prompt },
      usageDialog: !!document.querySelector(".session-usage-dialog"),
      disabledResetButtons: [...document.querySelectorAll<HTMLButtonElement>(".session-usage-dialog button")]
        .filter(node => node.textContent?.includes("Spend one saved reset")).map(node => node.disabled),
    };
  },
});
