const { assertCaptureTransition } = require('./capture-state.cjs');
...
function createSurfaceGuard({ window, output, fixture, conditions, http }) {
  ...
  const fail = (reason, observed) => {
    ...
    throw new Error('Reset dialog surface stopped: ' + failed.reason);
  };
  ...
  const geometry = async (label, requireFocus = true) => {
    if (failed) throw new Error('Reset dialog surface stopped: ' + failed.reason);
  ...
  return {
    ...
    check,
    async capture(label, file, expectedState) {
      const before = await check(label + ':before-capture');
      const record = { label, file, qualified: false, before, expectedState, referenceComparison: { matched: false, reason: 'No independently matched reference zoom/aspect/theme/profile conditions are supplied by this functional fixture.' } };
      try {
        ...
        record.after = await check(label + ':after-capture');
        const current = { state: expectedState, sha256: record.png.sha256 };
        assertCaptureTransition(previousCapture, current);
        previousCapture = current;
        record.qualified = true; return record;
