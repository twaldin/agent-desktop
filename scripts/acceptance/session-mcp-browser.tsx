...
async function request(route:string,body?:unknown){const response=await fetch(endpoint+route,{...(body?{method:'POST',body:JSON.stringify(body),headers:{'Content-Type':'application/json'}}:{})});if(!response.ok)throw new Error('Controlled MCP endpoint unavailable');return response.json();}
const bridge={getSessionMcp:(_session:string,_host:string,commandId?:string)=>request('/read'+(commandId?'?commandId='+encodeURIComponent(commandId):'')),command:(envelope:unknown)=>request('/reload',envelope)} as DesktopBridge;
