...
const bridge={subscribe:()=>()=>{},getPlugins:(target,hostId)=>request('/plugins/read',{target}),mutatePlugin:(target,mutation)=>request('/plugins/mutate',{target,mutation}),getMcpServers:target=>request('/mcp/read',{target}),getMcpServerDetail:(target,requestBody)=>request('/mcp/detail',{target,request:requestBody}),mutateMcpServer:(target,mutation)=>request('/mcp/mutate',{target,mutation}),openExternal:async(url:string)=>{docs.push(url);}} satisfies Pick<DesktopBridge,'subscribe'|'getPlugins'|'mutatePlugin'|'getMcpServers'|'getMcpServerDetail'|'mutateMcpServer'|'openExternal'>;
