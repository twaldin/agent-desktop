...
async function exercise(window) {
  ...
  try {
    ...
    await wait(`(${copyButton})?.disabled===true&&(${copyButton}).title.includes('Wait for the current copy')`, 'pending clipboard attempt on another conversation has an explicit reason');
    ...
    checks.push('real native journals, retained file and exact draft revisions unchanged');
    if (errors.length) throw new Error('Renderer reported errors during conversation copy: ' + errors.join('; '));
    passed = true;
