...
function createSurfaceGuard({ window, output, fixture, conditions, http }) {
  ...
  let windowId, displayId, displayBaseline, spaceId, themeBaseline, failed, prepared = false;
  ...
  const fail = (reason, observed) => {
    ...
    throw new Error('Conversation Markdown surface stopped: ' + failed.reason);
  };
  ...
  const geometry = async (label, requireFocus = true) => {
    if (failed) throw new Error('Conversation Markdown surface stopped: ' + failed.reason);
    ...
    if (!display || display.backingScaleFactor !== conditions.dpr || !(display.modeWidth > 0 && display.modeHeight > 0 && display.pixelWidth > 0 && display.pixelHeight > 0)
    ...
    if (JSON.stringify(display) !== displayBaseline) return fail('The verified native display mode or backing geometry changed.', observed);
    ...
    if (managed.running && managed.window.space !== spaceId) return fail('The owned window changed its verified Space.', observed);
  ...
  return {
    async prepare() {
      ...
      displayBaseline = JSON.stringify(display);
      const managed = await tiler(windowId);
      spaceId = managed.running ? managed.window.space : undefined;
      if (managed.running && (!Number.isSafeInteger(spaceId) || spaceId < 1)) return fail('The owned native Space cannot be verified.', managed);
