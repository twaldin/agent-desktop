...
const repository = resolve(import.meta.dir, '../../..'), output = resolve(process.argv[2] ?? '.data/conversation-markdown-app');
if (process.env.CONVERSATION_MARKDOWN_CLIPBOARD_BOUNDARY !== 'controlled-api') throw new Error('Explicit controlled-api clipboard boundary approval is required; OS clipboard acceptance is not implemented by this fixture.');
if (process.platform !== 'darwin') throw new Error('Native conversation-markdown acceptance requires macOS CG/AX inspection.');
for (const path of ['main.cjs', 'preload.cjs', 'renderer/index.html']) await access(join(repository, 'apps/desktop/dist', path));
await mkdir(output, { recursive: true });
if ((await readdir(output)).length) throw new Error('Refusing to overwrite conversation-markdown evidence.');
const conditions: SurfaceConditions = process.argv[3] ? JSON.parse(await readFile(resolve(process.argv[3]), 'utf8')) : {
  name: 'conversation-markdown-main-1440x1000', responsive: false, bounds: { x: 560, y: 180, width: 1440, height: 1000 },
...
const sources = ['apps/desktop/src/renderer/conversation-markdown.ts', 'apps/desktop/src/renderer/conversation-markdown.test.ts', 'apps/desktop/src/renderer/desktop-state.ts',
  'apps/desktop/src/renderer/App.tsx', 'apps/desktop/src/renderer/app-command-bindings.ts', 'apps/desktop/src/renderer/app-shortcuts.ts', 'apps/desktop/src/renderer/styles.css', 'apps/desktop/src/renderer/theme.css',
  ...['host.ts', 'run.ts', 'main.cjs', 'surface-guard.cjs', 'geometry.swift'].map(name => `scripts/acceptance/conversation-markdown-app/${name}`)];
...
const fixture = await realpath(await mkdtemp(join(tmpdir(), 'agent-desktop-conversation-markdown-'))), privateBin = join(fixture, 'bin');
...
const environment = { HOME: fixture, USER: username, LOGNAME: username, PATH: privateBin + delimiter + (process.env.PATH ?? '/usr/bin:/bin:/usr/sbin:/sbin'), TMPDIR: tmpdir(), PI_DISABLE_DOTENV: '1',
  PI_CODING_AGENT_DIR: join(fixture, 'agent'), AGENT_DESKTOP_DATA_DIR: join(fixture, 'data'), AGENT_DESKTOP_PROFILE_DIR: join(fixture, 'profile'), AGENT_DESKTOP_NATIVE_TERMINALS: '0', TERM: 'dumb', CONVERSATION_MARKDOWN_CLIPBOARD_BOUNDARY: 'controlled-api' };
...
try {
  ...
  while (!await Bun.file(join(fixture, 'ready.json')).exists()) {
    if (host.exitCode !== null || Date.now() > deadline) throw new Error('Markdown fixture host did not become ready.');
    await Bun.sleep(50);
  }
  const { connection, sessions } = JSON.parse(await readFile(join(fixture, 'ready.json'), 'utf8')) as { connection: LocalConnection; sessions: { id: string }[] };
  ...
  const saved = state.saveView({ ...defaultWindowView(), route: { hostId: connection.hostId, sessionId: sessions[0]!.id }, settingsOpen: false, workspaceOpen: false, collapsedSidebarSections: [],
    dock: { tabs: [], state: { ...createDockState(), right: { tabIds: [], open: false } } } });
  if (saved.error) throw new Error(saved.error);
  await writeFile(join(candidate, 'package.json'), JSON.stringify({ name: 'agent-desktop-conversation-markdown-acceptance', version: '0.0.0', main: 'main.cjs' }));
  for (const name of ['main.cjs', 'surface-guard.cjs']) await copyFile(join(import.meta.dir, name), join(candidate, name));
  ...
  const timer = setTimeout(() => electron!.kill('SIGTERM'), 600_000), code = await electron.exited; clearTimeout(timer);
  if (code !== 0) throw new Error(`Conversation-Markdown acceptance is RED (${code}); inspect ${output}/result.json.`);
  if (!JSON.parse(await readFile(join(output, 'result.json'), 'utf8')).passed) throw new Error('Conversation-Markdown acceptance did not pass.');
} finally {
  ...
  const sourceUnchanged = JSON.stringify(before) === JSON.stringify(after), fixtureRemoved = code === 0 && electron?.exitCode === 0 && sourceUnchanged;
  if (fixtureRemoved) await rm(fixture, { recursive: true, force: true });
  await writeFile(join(output, 'cleanup.json'), JSON.stringify({ hostExit: code, electronExit: electron?.exitCode, fixtureRemoved, retainedFixture: fixtureRemoved ? undefined : fixture, sourceUnchanged }));
  if (code !== 0 || JSON.stringify(before) !== JSON.stringify(after)) throw new Error('Markdown fixture cleanup or source freeze failed.');
}
