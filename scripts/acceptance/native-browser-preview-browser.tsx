...
import type { BrowserFrameSnapshot, BrowserFrameTarget, BrowserMetadataSnapshot, DesktopBridge } from "../../packages/shared/src/protocol";
...
declare global {
  interface Window {
    nativePreviewBridge: {
      ...
      capture(label: "initial" | "remounted"): Promise<string>;
    };
...
window.nativeBrowserPreviewProgress = () => ({ checks, metadataCalls, frameCalls, text: document.body.innerText, lastMetadata,
...
window.runNativeBrowserPreviewAcceptance = async () => {
  ...
  root.render(null);
  return { passed: true, checks, expected, metadataCalls, frameCalls, decodedJpeg: { width: 640, height: 480, sha256: initialDigest, base64Characters: lastFrame.data.length },
