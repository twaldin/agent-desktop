...
Object.assign(window, {
  ...
  awaitSetupRunning: async () => { await wait(() => document.querySelector('.environment-preparation-heading')?.textContent?.includes('Setting up environment'), 'running setup', 25_000); assert(exactButton('Cancel', document.querySelector('.environment-preparation-card')!), 'Running setup has no Cancel control'); checks.push('running setup exposes cancellation without waiting for create to finish'); return state(); },
  awaitSetupOutput: async () => { await wait(() => document.querySelector('.environment-output-scroll')?.textContent?.includes('SETUP_WAITING_FOR_CANCEL'), 'live setup output'); assert(document.querySelector('.environment-output-scroll')?.textContent?.includes('Setup diagnostic detail'), 'Missing standard error'); assert(document.querySelector('.environment-preparation-heading')?.textContent?.includes('Setting up environment'), 'Setup finished before live output proof'); checks.push('explicit details show real stdout and stderr while setup is still running'); return state(); },
