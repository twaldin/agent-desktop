...
const methods: Partial<DesktopBridge> = {
  ...
  getState: async requested => structuredClone(requested === foreignOwner && foreignState ? foreignState : state), getHosts: async () => ({ status: "connected", ownNodeId: "controlled", checkedAt: 1, hosts: [] }),
  getPreferences: async () => ({ version: 1, records: structuredClone(preferenceRecords) }), getPreferencesV2: async () => ({ version: 2 as const, records: structuredClone(preferenceRecords) }), getTheme: async () => ({ document: { ...DEFAULT_THEME, mode: "dark" }, revision: "theme", filePath: "/controlled/theme.json" }),
...
Object.assign(window,{runBrowserAutocompleteAppAcceptance:async()=>{
  ...
  await waitApp(()=>windowState.route.hostId===foreignOwner,"foreign host route");
  const retainedBrowser=document.querySelector<HTMLElement>('.dock-slot-right .browser-panel');
  assertApp(retainedBrowser&&retainedBrowser.getClientRects().length&&!retainedBrowser.closest('[inert]'),"The independently retained browser pane was not visible beside the foreign chat");
  await window.fixtureApi.call('browserCallMarker',['foreign-visible-owner']);
  const shell=document.querySelector<HTMLElement>('.app-shell')!;shell.style.width=`${Math.max(620,Math.floor(shell.getBoundingClientRect().width)-90)}px`;
  await waitAsync(async()=>{const trace=await window.fixtureApi.call('browserCallTrace',[]),marker=trace.findIndex((entry:any)=>entry.method==='MARKER'&&entry.args?.[0]==='foreign-visible-owner');return marker>=0&&trace.slice(marker+1).some((entry:any)=>entry.method==='controlBrowser'&&entry.args?.[0]===sessionId&&entry.args?.[2]===owner&&entry.args?.[1]?.action?.type==='resize')},"retained browser owner resize");
  const foreignTrace=await window.fixtureApi.call('browserCallTrace',[]),foreignMarker=foreignTrace.findIndex((entry:any)=>entry.method==='MARKER'&&entry.args?.[0]==='foreign-visible-owner');
  assertApp(foreignMarker>=0,"Visible-owner trace marker missing");
  const foreignCalls=foreignTrace.slice(foreignMarker+1).filter((entry:any)=>entry.method!=='MARKER');
  assertApp(foreignCalls.length>0&&foreignCalls.every((entry:any)=>entry.args?.[0]===sessionId&&entry.args?.at(-1)===owner),"Visible retained browser retargeted to the foreign chat owner");
  const sideOpen=dockToggle("side",true);assertApp(sideOpen,"Open side dock toggle missing");sideOpen.click();
  await waitApp(()=>{const slot=document.querySelector<HTMLElement>('.dock-slot-right');return Boolean(slot?.hasAttribute('inert')&&getComputedStyle(slot).display==='none')},"browser pane hidden and inert");
  await new Promise(resolve=>setTimeout(resolve,250));const callsAtHidden=await window.fixtureApi.call('browserCallCount',[]);await new Promise(resolve=>setTimeout(resolve,1300));
  assertApp((await window.fixtureApi.call('browserCallCount',[]))===callsAtHidden,"Hidden browser pane kept issuing browser work");
  ...
  const sideClosed=dockToggle("side",false);assertApp(sideClosed,"Closed side dock toggle missing");sideClosed.click();
  await waitApp(()=>{const panel=document.querySelector<HTMLElement>('.dock-slot-right .browser-panel');return Boolean(panel&&panel.getClientRects().length&&!panel.closest('[inert]'))},"original browser dock restored");
  return{passed:true,route:windowState.route,checks:['dock open','fast Enter','history delete/restart','Back/Forward/Stop','offline/reconnect','foreign chat with retained original browser','hidden browser inactivity'],scope:'Production App with controlled catalog/dock state and actual authenticated OMP CDP browser transports. Host connection, retained-pane geometry, and second-host events are controlled; native page/navigation/history are real. No provider, personal Chrome history, physical focus, native search-engine autocomplete, or cmux proof.'};
}});
