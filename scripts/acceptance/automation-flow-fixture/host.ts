...
globalThis.fetch = Object.assign(async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]) => {
  ...
  if (url.hostname !== '127.0.0.1') throw new Error('Outbound provider/network fetch forbidden in Automation flow fixture.');
  return originalFetch(input, init);
}, { preconnect: () => {} }) as typeof fetch;
for (const path of ['data', 'agent', 'project', 'project/.omp/skills/flow-review']) await mkdir(join(fixture, path), { recursive: true });
await writeFile(join(fixture, 'project', 'README.md'), '# Automation flow fixture\n');
await writeFile(join(fixture, 'project', '.omp/skills/flow-review/SKILL.md'), '---\nname: flow-review\ndescription: Review this disposable workspace\n---\nRead the source carefully.\n');
await writeFile(join(fixture, 'project', 'file with spaces.txt'), 'Exact owner file contents\n');
await writeFile(join(fixture, 'agent', 'config.yml'), `extensions:\n  - ${JSON.stringify(resolve(import.meta.dir, 'extension.ts'))}\ntools:\n  approvalMode: always-ask\n`);
