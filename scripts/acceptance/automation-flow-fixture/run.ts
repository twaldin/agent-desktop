...
const root = resolve(import.meta.dir, '../../..'), output = resolve(process.argv[2] ?? '.data/automation-flow-fixture-001');
...
const sourcePaths = [...new Set([...(await new Response(tracked.stdout).text()).split('\0').filter(Boolean), ...(await readdir(import.meta.dir)).map(name => `scripts/acceptance/automation-flow-fixture/${name}`)])].sort();
...
try {
  ...
  const project = await command({ type: 'project.add', path: join(fixture, 'project'), name: 'Automation workspace' });
  const session = await command({ type: 'session.create', projectId: project.id });
  await writeFile(join(fixture, 'context.json'), JSON.stringify({ projectId: project.id, sessionId: session.id }));
  ...
  await writeFile(join(output, 'preload.cjs'), `const {contextBridge,ipcRenderer}=require('electron');contextBridge.exposeInMainWorld('automationFlowFixture',{save:value=>ipcRenderer.sendSync('automation-flow-save',value),call:(method,args)=>ipcRenderer.invoke('automation-flow-call',method,args),subscribe:listener=>{const handler=(_event,value)=>listener(value);ipcRenderer.on('automation-flow-event',handler);return()=>ipcRenderer.removeListener('automation-flow-event',handler);}});`);
