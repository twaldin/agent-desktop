...
try{
 ...
 host=Bun.spawn([process.execPath,join(repo,'apps/host/src/fixtures/mcp-authorization-route.ts'),root,process.argv[3]==='--tool'?'--ui-tool':process.argv[3]==='--conversation'?'--ui-slash':'--ui'],{cwd:root,env:{HOME:root,PI_CODING_AGENT_DIR:join(root,'agent'),MCP_CONTRACT_GATES:join(root,'gates'),PATH:process.env.PATH,SHELL:'/bin/sh',TMPDIR:tmpdir(),TERM:'dumb'},stdout:Bun.file(join(output,'host.log')),stderr:Bun.file(join(output,'host-error.log'))});
