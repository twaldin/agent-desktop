...
const output = resolve(process.argv[2] ?? `.data/window-close-acceptance-${Date.now()}`);
const sources = ["apps/desktop/src/main/window-close.ts", "apps/desktop/src/main/main.ts", "apps/desktop/src/main/preload.ts", "apps/desktop/src/renderer/WindowClose.tsx", "apps/desktop/src/renderer/window-close.css", "apps/desktop/src/renderer/native-skill-file-state.ts",
  "apps/desktop/src/renderer/App.tsx", "apps/desktop/src/renderer/DockPanel.tsx", "apps/desktop/src/renderer/dock-state.ts", "apps/desktop/src/renderer/dock-panel.css", "apps/desktop/src/renderer/WorkspacePanel.tsx", "apps/desktop/src/renderer/WorkspaceFileClose.tsx", "apps/desktop/src/renderer/workspace-state.ts", "apps/desktop/src/renderer/workspace-lease.ts",
  ...
  "scripts/acceptance/window-close.ts", "scripts/acceptance/workspace-file-host.ts", "scripts/acceptance/window-close-browser.tsx", "scripts/acceptance/window-close-electron.cjs",
];
...
const fixture = await realpath(await mkdtemp(join(tmpdir(), "agent-desktop-window-close-")));
...
try {
  ...
  const calls: Array<{ route: string; command?: string; action?: string; id?: string; path?: string; text?: string; expectedRevision?: string | null; arrivedAt?: number; releasedAt?: number; droppedReceipt?: boolean }> = [], pendingEvents: unknown[] = [];
  let gate: { path: string; count: number; releases: Map<number, () => void> } | undefined, dropNextReceipt = false;
  ...
  proxy = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(request) {
    ...
    if (route === "/test/gate-two-writes") { fixturePath(input.file); gate = { path: input.file, count: 0, releases: new Map() }; return Response.json({ armed: true }, { headers: cors }); }
    if (route === "/test/release-write") { const release = gate?.releases.get(input.ordinal); if (!release) return new Response(null, { status: 409, headers: cors }); gate!.releases.delete(input.ordinal); release(); return Response.json({ released: input.ordinal }, { headers: cors }); }
    if (route === "/test/drop-next-receipt") { dropNextReceipt = true; return Response.json({ armed: true }, { headers: cors }); }
    if (route === "/test/state") {
      ...
      const writes = calls.filter(call => call.command === "workspace.mutate" && call.action === "file.write");
      return Response.json({ calls, workspaceDeliveries: writes.length, uniqueWorkspaceCommands: new Set(writes.map(call => call.id)).size, gateArrivals: gate?.count ?? 0, sessions: nativeState.sessions?.length,
    ...
    const action = input.command?.action, call: (typeof calls)[number] = { route, command: input.command?.type, action: action?.type, id: input.id, path: action?.path, text: action?.text, expectedRevision: action?.expectedRevision, arrivedAt: Date.now() }; calls.push(call);
    const activeGate = gate;
    if (route === "/v1/commands" && activeGate && activeGate.path === action.path && activeGate.count < 2) { const ordinal = ++activeGate.count; await new Promise<void>(resolve => activeGate.releases.set(ordinal, resolve)); call.releasedAt = Date.now(); }
    ...
    if (route === "/v1/commands" && dropNextReceipt) { dropNextReceipt = false; call.droppedReceipt = true; return Response.json({ error: { message: "Fixture dropped the completed host receipt." } }, { status: 503, headers: cors }); }
    return new Response(body, { status: response.status, headers: { ...cors, "Content-Type": "application/json" } });
  }});
  await writeFile(join(output, "index.html"), `<!doctype html><meta charset="utf-8"><style>.workspace-file-fixture{display:flex;height:100vh;min-width:0}.workspace-file-fixture>.workspace-panel{position:relative;flex:1;width:auto;min-width:0}</style><div id="root"></div><script type="module" src="${relative(output, join(import.meta.dir, "window-close-browser.tsx"))}"></script>`);
  ...
  const bundled=await Bun.build({entrypoints:[join(repo,"apps/desktop/src/main/window-close.ts"),join(repo,"apps/desktop/src/main/preload.ts")],outdir:output,target:"node",format:"cjs",external:["electron"],naming:"[name].cjs"});if(!bundled.success)throw Error("Close module build failed");
  const electron = Bun.spawn([electronExecutable as unknown as string, join(import.meta.dir, "window-close-electron.cjs"), output], { stdout: Bun.file(join(output, "electron.log")), stderr: Bun.file(join(output, "electron-errors.log")) });
  ...
  await writeFile(join(output, "result.json"), JSON.stringify(result, null, 2)); if (!result.passed) throw new Error(`Window close acceptance failed; inspect ${join(output, "result.json")}`); console.log(JSON.stringify({ passed: true, output }));
