...
Object.assign(window,{
 ...
 state(){return{width:innerWidth,height:innerHeight,dpr:devicePixelRatio,theme:document.documentElement.dataset.theme,body:document.body.innerText,font:getComputedStyle(document.body).fontFamily,fontSize:getComputedStyle(document.body).fontSize,scrollWidth:document.documentElement.scrollWidth,active:document.activeElement?.getAttribute('aria-label'),activeControl:{tag:document.activeElement?.tagName,text:document.activeElement?.textContent,role:document.activeElement?.getAttribute('role')},storage:Object.fromEntries(Object.keys(localStorage).map(k=>[k,localStorage.getItem(k)])),geometry:[...document.querySelectorAll<HTMLElement>('.marketplace-dialog,.acquisition-menu,.marketplace-row,.integration-content')].filter(x=>x.getClientRects().length).map(x=>{const r=x.getBoundingClientRect(),s=getComputedStyle(x);return{class:x.className,x:r.x,y:r.y,width:r.width,height:r.height,background:s.backgroundColor,color:s.color,radius:s.borderRadius,font:s.fontFamily,fontSize:s.fontSize};})};}
});
