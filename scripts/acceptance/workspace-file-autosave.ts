...
const output = resolve(process.argv[2] ?? `.data/workspace-file-autosave-acceptance-${Date.now()}`);
const mode = process.argv.includes("--dock-close") ? "dock-close" : "core";
const sources = [
  ...
  "scripts/acceptance/workspace-file-autosave.ts", "scripts/acceptance/workspace-file-host.ts", "scripts/acceptance/workspace-file-autosave-browser.tsx", "scripts/acceptance/workspace-file-autosave-electron.cjs",
];
...
const fixture = await realpath(await mkdtemp(join(tmpdir(), "agent-desktop-workspace-file-autosave-")));
...
try {
  ...
  await writeFile(join(output, "index.html"), `<!doctype html><meta charset="utf-8"><style>.workspace-file-fixture{display:flex;height:100vh;min-width:0}.workspace-file-fixture>.workspace-panel{position:relative;flex:1;width:auto;min-width:0}</style><div id="root"></div><script type="module" src="${relative(output, join(import.meta.dir, "workspace-file-autosave-browser.tsx"))}"></script>`);
  ...
  await writeFile(join(output, "launch.json"), JSON.stringify({ endpoint: `http://127.0.0.1:${proxy.port}/${capability}`, target: ready.target, hostId: ready.connection.hostId, files: ready.files, profile: join(fixture, "electron-profile"), mode }), { mode: 0o600 });
  const electron = Bun.spawn([process.execPath, join(repo, "node_modules/electron/cli.js"), join(import.meta.dir, "workspace-file-autosave-electron.cjs"), output], { stdout: Bun.file(join(output, "electron.log")), stderr: Bun.file(join(output, "electron-errors.log")) });
  ...
  await writeFile(join(output, "result.json"), JSON.stringify(result, null, 2)); if (!result.passed) throw new Error(`Workspace file autosave acceptance failed; inspect ${join(output, "result.json")}`); console.log(JSON.stringify({ passed: true, output }));
