...
if (
  ...
  process.env.PLUGIN_RELOAD_FIXTURE_DIRECTORY !== fixture ||
  process.env.PATH?.split(delimiter)[0] !== bin
)
  throw new Error("An isolated plugin-reload App environment is required.");
...
await writeFile(join(fixture, "agent", "config.yml"), `extensions:\n  - ${JSON.stringify(resolve(import.meta.dir, "../../../apps/host/src/omp-workers/fixtures/ask-provider.ts"))}\ncodexResets:\n  autoRedeem: yes\n`);
...
const { getInstalledPluginsRegistryPath } = await import("@oh-my-pi/pi-coding-agent/extensibility/plugins/marketplace");
const userRegistry = getInstalledPluginsRegistryPath();
await writeFile(join(fixture, "user-registry-path"), userRegistry);
const pluginEntry = (scope: "user" | "project", installPath: string) => ({
  scope, installPath, version: "1.0.0", installedAt: "2026-09-19T00:00:00.000Z", lastUpdated: "2026-09-19T00:00:00.000Z", enabled: true,
});
for (const [scope, registry, installPath] of [
  ["user", userRegistry, join(fixture, "plugins", "user")],
  ["project", join(fixture, "project", ".omp", "plugins", "installed_plugins.json"), join(fixture, "plugins", "project")],
] as const) {
  await mkdir(installPath, { recursive: true }); await mkdir(resolve(registry, ".."), { recursive: true });
  await writeFile(join(installPath, "package.json"), JSON.stringify({ name: `fixture-${scope}`, version: "1.0.0" }));
  await writeFile(registry, JSON.stringify({ version: 2, plugins: { "fixture@local": [pluginEntry(scope, installPath)] } }, null, 2));
}
...
globalThis.fetch = Object.assign(async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (url.hostname !== "127.0.0.1") throw new Error("Nonlocal network is forbidden in the plugin-reload App fixture.");
