...
const root = resolve(import.meta.dir, "../../.."),
  output = resolve(process.argv[2] ?? ".data/plugin-reload-app-001");
await mkdir(output, { recursive: true });
if ((await readdir(output)).length)
  throw new Error("Refusing to overwrite prior fixture evidence.");
const sourcePaths = [
  "apps/desktop/src/renderer/App.tsx", "apps/desktop/src/main/host-transport.ts", "apps/desktop/src/main/command-endpoints.ts", "apps/desktop/src/main/composer-actions-transport.ts",
  "apps/host/src/server.ts", "apps/host/src/omp/commands.ts", "apps/host/src/omp/composer-actions.ts", "apps/host/src/omp/runtime.ts",
  "apps/host/src/omp-workers/runtime.ts", "apps/host/src/omp-workers/entry.ts", "apps/host/src/omp-workers/protocol.ts", "apps/host/src/omp-workers/fixtures/ask-provider.ts",
  "packages/shared/src/protocol.ts", "packages/shared/src/composer-actions.ts", "docs/native-plugin-reload.md",
  ...(await readdir(import.meta.dir)).map(name => `scripts/acceptance/plugin-reload-app/${name}`),
].sort();
const hashes = async () =>
  Object.fromEntries(
    await Promise.all(
...
const fixture = await realpath(
    await mkdtemp(join(tmpdir(), "agent-desktop-plugin-reload-app-")),
  ),
...
const host = Bun.spawn(
  [process.execPath, join(import.meta.dir, "host.ts"), fixture],
  {
    env: {
      ...
      PLUGIN_RELOAD_FIXTURE_DIRECTORY: fixture,
      OMP_EXTENSION_FACTORY_COUNT: join(fixture, "extension-factory-count"),
      PI_DISABLE_DOTENV: "1",
      TERM: "dumb",
...
try {
  ...
  await writeFile(
    ...
    `const{contextBridge,ipcRenderer}=require('electron');contextBridge.exposeInMainWorld('pluginReloadAppFixture',{save:value=>ipcRenderer.sendSync('plugin-reload-app-save',value),call:(method,args)=>ipcRenderer.invoke('plugin-reload-app-call',method,args),subscribe:listener=>{const handler=(_event,value)=>listener(value);ipcRenderer.on('plugin-reload-app-event',handler);return()=>ipcRenderer.removeListener('plugin-reload-app-event',handler);}});`,
  );
  electron = Bun.spawn(
    [
      process.execPath,
