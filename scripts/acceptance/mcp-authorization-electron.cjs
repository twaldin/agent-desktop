...
app.whenReady().then(async()=>{
 ...
 try{
  await win.loadFile(path.join(output,'web/index.html'),{query:{hostId:ready.endpoint.hostId,sessionId:ready.sessionId,conversation:String(Boolean(ready.conversation))}});win.webContents.focus();
  if(!ready.conversation){await wait(`[...document.querySelectorAll('button')].some(x=>x.textContent==='Authenticate'&&!x.disabled)`);await capture('01-authenticate');await click('Authenticate');}
  await wait(`document.querySelector('.secret-form input') && document.body.innerText.includes('Open sign-in page')`);await capture('02-pending');
  await win.webContents.reload();await wait(`document.querySelector('.secret-form input')`);if(starts!==(ready.conversation?0:1))throw new Error('Reload replayed authorization');
  ...
  await win.webContents.reload();if(ready.conversation){await new Promise(r=>setTimeout(r,2200));await wait(`!document.querySelector('.pending-mcp-authorization')`);}else await wait(`document.body.innerText.includes('Server connected')`);await capture('05-reopen');
  if(starts!==(ready.conversation?0:1)||answers!==1)throw new Error('Duplicate operation');if(JSON.stringify(captures.map(x=>x.state.storage)).includes('route-private-code'))throw new Error('Private callback cached');
  fs.writeFileSync(path.join(output,'result.json'),JSON.stringify({passed:true,starts,answers,opened,slash:Boolean(ready.slash),tool:Boolean(ready.tool),conversation:Boolean(ready.conversation),captures,scope:'Hidden Electron real renderer and production transport over fixture IPC, authenticated real host/native local MCP OAuth. No full-app IPC, native OS-window, hosted-provider or pixel-parity claim.'},null,2));fs.writeFileSync(path.join(root,'ui-done'),'done');app.exit(0);
