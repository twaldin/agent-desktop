...
try {
  ...
  result.passed &&= exitCode === 0 && result.sourceHashesStable && markerText === "executed\nexecuted\n" && list.terminals.length === 1 && actionTerminalIds.length === 2 && new Set(actionTerminalIds).size === 1 && actionTerminalIds[0] === list.terminals[0]?.id && result.captures?.length === 4 && result.state?.checks?.length === 7;
