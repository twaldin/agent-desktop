...
const root = resolve(import.meta.dir, '../../..'), output = resolve(process.argv[2] ?? '.data/composer-flow-fixture-001');
...
const sourcePaths = [...new Set([...(await new Response(tracked.stdout).text()).split('\0').filter(Boolean), ...(await readdir(import.meta.dir)).map(name => `scripts/acceptance/composer-flow-fixture/${name}`)])].sort();
...
const fixture = await realpath(await mkdtemp(join(tmpdir(), 'agent-desktop-composer-flow-')));
...
try {
  ...
  const project = await command({ type: 'project.add', path: join(fixture, 'project'), name: 'Composer workspace' });
  ...
  await writeFile(join(output, 'preload.cjs'), `const {contextBridge,ipcRenderer}=require('electron');contextBridge.exposeInMainWorld('composerFlowFixture',{call:(method,args)=>ipcRenderer.invoke('composer-flow-call',method,args),subscribe:listener=>{const handler=(_event,value)=>listener(value);ipcRenderer.on('composer-flow-event',handler);return()=>ipcRenderer.removeListener('composer-flow-event',handler);}});`);
