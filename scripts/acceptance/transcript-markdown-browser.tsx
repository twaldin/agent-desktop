...
export async function transcriptMarkdownAcceptance() {
  ...
  try {
    ...
    const file = container.querySelector<HTMLButtonElement>(".markdown-file-link")!; file.click(); await settle(); check(JSON.stringify(files[0]) === JSON.stringify({ path: "src/example.ts", line: 3, column: 2 }), "file callback receives owner-relative path and exact position");
