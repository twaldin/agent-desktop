...
const root = resolve(import.meta.dir, "../../.."),
  output = resolve(process.argv[2] ?? ".data/session-usage-app-001");
...
const sourcePaths = [
  ...
  "docs/native-provider-usage-flow.md",
  ...(await readdir(import.meta.dir)).map(name => `scripts/acceptance/session-usage-app/${name}`),
].sort();
const hashes = async () =>
  Object.fromEntries(
    await Promise.all(
...
const fixture = await realpath(
    await mkdtemp(join(tmpdir(), "agent-desktop-session-usage-app-")),
  ),
...
try {
  ...
  const project = await command({
    ...
    session = await command({ type: "session.create", projectId: project.id, model: { provider: "usage-fixture", id: "fixture" } });
  await writeFile(
  ...
  await writeFile(
    ...
    `const{contextBridge,ipcRenderer}=require('electron');contextBridge.exposeInMainWorld('sessionUsageAppFixture',{save:value=>ipcRenderer.sendSync('session-usage-app-save',value),call:(method,args)=>ipcRenderer.invoke('session-usage-app-call',method,args),subscribe:listener=>{const handler=(_event,value)=>listener(value);ipcRenderer.on('session-usage-app-event',handler);return()=>ipcRenderer.removeListener('session-usage-app-event',handler);}});`,
  );
  electron = Bun.spawn(
    [
      process.execPath,
  ...
  for (const name of ["worker-starts.jsonl", "held-read.jsonl", "usage-pids.jsonl"]) {
