...
const createEndpoint = new BrowserCreateHttp({ hostId, controlEpoch: controlEndpoint.epoch, sessionExists: id => id === session?.id, getHandle: async id => { if (id !== session?.id) throw new Error("Missing session"); return session; }, getExistingHandle: async id => id === session?.id ? session : undefined });
...
try {
  ...
  await writeFile(join(output, "main.cjs"), `const{app,BrowserWindow,ipcMain}=require('electron'),fs=require('node:fs'),path=require('node:path');const{requestBrowserCreate}=require('./transport/browser-create-transport.cjs');const{requestBrowserMetadata}=require('./transport/browser-metadata-transport.cjs'),{requestBrowserFrame}=require('./transport/browser-frame-transport.cjs'),{requestBrowserControl}=require('./transport/browser-control-transport.cjs');
