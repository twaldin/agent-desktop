...
import { BrowserCreationRecords } from "../../apps/host/src/browser-creation-records";
...
const controlMode = createMode || process.argv.includes("--controls");
const sourceFiles = [
  "apps/desktop/src/main/window-state.ts", "apps/desktop/src/renderer/window-view-state.ts", "apps/desktop/src/renderer/browser-window-checkpoint.ts", "apps/desktop/src/renderer/browser-new-tab.ts",
  "apps/host/src/browser-create-http.ts", "apps/host/src/browser-creation-records.ts", "apps/desktop/src/main/browser-create-transport.ts", "packages/shared/src/browser-create.ts",
...
const metadataEndpoint = new BrowserMetadataHttp({ hostId, creationTicket: () => ({controlEpoch: controlEndpoint.epoch, observedAt: Date.now()}), sessionExists: id => id === session?.id, getExistingHandle: async id => id === session?.id ? session : undefined });
const controlEndpoint = new BrowserControlHttp({ hostId, sessionExists: id => id === session?.id, getExistingHandle: async id => id === session?.id ? session : undefined });
const creationDb = new Database(":memory:");
creationDb.exec("CREATE TABLE metadata(key TEXT PRIMARY KEY, data TEXT NOT NULL)");
const creationRecords = new BrowserCreationRecords(creationDb, hostId, () => creationDb.exec("PRAGMA user_version = 16"));
const createEndpoint = new BrowserCreateHttp({ records: creationRecords, hostId, controlEpoch: controlEndpoint.epoch, sessionExists: id => id === session?.id, getHandle: async id => { if (id !== session?.id) throw new Error("Missing session"); return session; }, getExistingHandle: async id => id === session?.id ? session : undefined });
const frameEndpoint = new BrowserFrameHttp({ hostId, controlEpoch: controlEndpoint.epoch, sessionExists: id => id === session?.id, getExistingHandle: async id => id === session?.id ? session : undefined });
const server = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(request) {
  const url = new URL(request.url);
  if (url.pathname === "/page" || url.pathname === "/second") return new Response(`<!doctype html><meta charset="utf-8"><title>Actual native preview</title><style>html,body{margin:0;width:100%;height:100%;background:#173f5f;color:white;font:24px system-ui}main{padding:32px}input{position:absolute;left:32px;top:80px;width:420px;height:40px;box-sizing:border-box}</style><main>Actual native viewport<input id="draft" value="unsent native input"><button id="counter" style="position:absolute;left:32px;top:160px;width:160px;height:40px">Count 0</button></main><script>document.cookie='native_preview_cookie=retained; SameSite=Lax';const draft=document.querySelector('#draft');globalThis.browserFrameState={get cookie(){return document.cookie},get unsentInput(){return draft.value},marker:'unchanged',count:0};document.querySelector('#counter').onclick=()=>{browserFrameState.count++;document.querySelector('#counter').textContent='Count '+browserFrameState.count};setInterval(()=>fetch('/heartbeat',{cache:'no-store'}).catch(()=>{}),100)</script>`, { headers: { "Content-Type": "text/html", "Set-Cookie": "native_preview_server=retained; SameSite=Lax", "Cache-Control": "no-store" } });
  ...
  if (url.pathname.startsWith("/v1/")) {
    ...
    const response = await metadataEndpoint.route(request, url) ?? await createEndpoint.route(request, url) ?? await frameEndpoint.route(request, url) ?? await controlEndpoint.route(request, url) ?? new Response("Not found", { status: 404 });
...
try {
  ...
  const transport = await Bun.build({ entrypoints: [join(repository, "apps/desktop/src/main/window-state.ts"), join(repository, "apps/desktop/src/main/browser-metadata-transport.ts"), join(repository, "apps/desktop/src/main/browser-frame-transport.ts"), join(repository, "apps/desktop/src/main/browser-control-transport.ts"), join(repository, "apps/desktop/src/main/browser-create-transport.ts")], outdir: join(output, "transport"), target: "node", format: "cjs", naming: "[name].cjs" });
  if (!transport.success) throw new Error(transport.logs.join("\n"));
  await writeFile(join(output, "preload.cjs"), `const{contextBridge,ipcRenderer}=require('electron');contextBridge.exposeInMainWorld('agentDesktopWindow',{initial:ipcRenderer.sendSync('native-preview:window-initial'),save:value=>ipcRenderer.sendSync('native-preview:window-save',value)});contextBridge.exposeInMainWorld('nativePreviewBridge',{createBrowserTab:(sessionId,request,hostId)=>ipcRenderer.invoke('native-preview:create',sessionId,request,hostId),getBrowserMetadata:(sessionId,hostId)=>ipcRenderer.invoke('native-preview:metadata',sessionId,hostId),getBrowserFrame:(sessionId,target,hostId)=>ipcRenderer.invoke('native-preview:frame',sessionId,target,hostId),insertText:text=>ipcRenderer.invoke('native-preview:insert-text',text),key:key=>ipcRenderer.invoke('native-preview:key',key),controlBrowser:(sessionId,request,hostId)=>ipcRenderer.invoke('native-preview:control',sessionId,request,hostId),capture:label=>ipcRenderer.invoke('native-preview:capture',label)});`, { mode: 0o600 });
  await writeFile(join(output, "main.cjs"), `const{app,BrowserWindow,ipcMain}=require('electron'),fs=require('node:fs'),path=require('node:path');const{WindowStateStore}=require('./transport/window-state.cjs');const{requestBrowserCreate}=require('./transport/browser-create-transport.cjs');const{requestBrowserMetadata}=require('./transport/browser-metadata-transport.cjs'),{requestBrowserFrame}=require('./transport/browser-frame-transport.cjs'),{requestBrowserControl}=require('./transport/browser-control-transport.cjs');
app.setPath('userData',${JSON.stringify(profile)});const output=${JSON.stringify(output)},endpoint={origin:${JSON.stringify(`http://127.0.0.1:${server.port}`)},hostId:${JSON.stringify(hostId)},token:${JSON.stringify(token)}};let window;const windowState=new WindowStateStore(${JSON.stringify(profile)},'native-preview');
ipcMain.on('native-preview:window-initial',event=>{event.returnValue=window&&event.sender===window.webContents?windowState.bootstrap():{error:'Unexpected fixture sender'}});ipcMain.on('native-preview:window-save',(event,value)=>{event.returnValue=window&&event.sender===window.webContents?windowState.saveView(value):{error:'Unexpected fixture sender'}});
ipcMain.handle('native-preview:create',(_event,sessionId,request,hostId)=>{if(hostId!==endpoint.hostId)throw new Error('Unexpected fixture owner');return requestBrowserCreate(endpoint,sessionId,request)});ipcMain.handle('native-preview:metadata',(_event,sessionId,hostId)=>{if(hostId!==endpoint.hostId)throw new Error('Unexpected fixture owner');return requestBrowserMetadata(endpoint,sessionId)});ipcMain.handle('native-preview:frame',(_event,sessionId,target,hostId)=>{if(hostId!==endpoint.hostId)throw new Error('Unexpected fixture owner');return requestBrowserFrame(endpoint,sessionId,target)});ipcMain.handle('native-preview:insert-text',(_event,text)=>{if(typeof text!=='string'||text.length>100)throw new Error('Invalid fixture text');return window.webContents.insertText(text)});ipcMain.handle('native-preview:key',(_event,key)=>{if(key!=='End')throw new Error('Invalid fixture key');window.webContents.sendInputEvent({type:'keyDown',keyCode:key});window.webContents.sendInputEvent({type:'keyUp',keyCode:key})});ipcMain.handle('native-preview:control',(_event,sessionId,request,hostId)=>{if(hostId!==endpoint.hostId)throw new Error('Unexpected fixture owner');return requestBrowserControl(endpoint,sessionId,request)});ipcMain.handle('native-preview:capture',async(_event,label)=>{if(!['initial','fit','remounted','controlled','created','restored'].includes(label))throw new Error('Invalid capture label');fs.writeFileSync(path.join(output,label+'.png'),(await window.webContents.capturePage()).toPNG());return label+'.png'});
app.whenReady().then(async()=>{window=new BrowserWindow({show:false,width:900,height:700,webPreferences:{preload:path.join(output,'preload.cjs'),sandbox:true,contextIsolation:true,nodeIntegration:false,backgroundThrottling:false}});try{await window.loadFile(path.join(output,'web/index.html'),{query:${JSON.stringify({ controls: String(controlMode), create: String(createMode), hostId, sessionId: session.id, workerPid: String(target.workerPid), name: target.name, targetId: target.targetId, url: tab.url, title: tab.title ?? "" })}});const value=await window.webContents.executeJavaScript('runNativeBrowserPreviewAcceptance()');fs.writeFileSync(path.join(output,'renderer-result.json'),JSON.stringify({...value,electron:process.versions.electron,hidden:true},null,2));window.destroy();app.exit(value.passed?0:1)}catch(error){const progress=await window.webContents.executeJavaScript('nativeBrowserPreviewProgress()').catch(()=>null);fs.writeFileSync(path.join(output,'renderer-result.json'),JSON.stringify({passed:false,error:String(error),progress},null,2));fs.writeFileSync(path.join(output,'failure.png'),(await window.webContents.capturePage()).toPNG());app.exit(1)}});`, { mode: 0o600 });
  ...
  const automaticFit = renderer.actions?.find((action: { type?: string }) => action.type === "resize")?.request?.action;
  if (states.length !== 2 || states[0].url !== tab.url || states[1].url !== tab.url
    || (controlMode && (automaticFit?.type !== "resize" || states[1].viewport?.width !== automaticFit.width || states[1].viewport?.height !== automaticFit.height))
    || (!controlMode && JSON.stringify(states[0].viewport) !== JSON.stringify(states[1].viewport))
    || states[0].state?.cookie !== states[1].state?.cookie || states[1].state?.unsentInput !== (controlMode ? "unsent native input typed through desktop" : "unsent native input") || states[1].state?.marker !== "unchanged") throw new Error("Native document state changed while the preview viewer was hidden or remounted");
  if (controlMode && states[1].state?.count !== 1) throw new Error("Desktop click did not update the same native counter exactly once");
  if (createMode) {
    const createdTarget = renderer.createdTarget;
    if (!createdTarget || createdTarget.workerPid !== session.workerPid) throw new Error("Created dock lost native worker identity");
    const inspectedCreated = session.startPrompt(`/inspect-browser-frame-contract ${createdTarget.name}`); await inspectedCreated.accepted; await inspectedCreated.completion;
    const latestEntries = (await readFile(session.sessionFile, "utf8")).trim().split("\n").map(line => JSON.parse(line));
    const createdState = latestEntries.filter(entry => entry.type === "custom" && entry.customType === "browser-frame-contract-state").at(-1)?.data;
    if (createdState?.url !== `http://127.0.0.1:${server.port}/second` || createdState?.state?.marker !== "unchanged") throw new Error("Created dock did not navigate the actual native page");
    renderer.createdNativeState = createdState;
  }
  ...
  result = { passed: true, scope: "Actual isolated OMP worker and tab through authenticated Bun HTTP, production Electron main transports, minimal preload, and production BrowserPanel", renderer,
    ...
    http: { requestCount: requests.length, requests }, captures: createMode ? ["initial.png", "fit.png", "remounted.png", "controlled.png", "created.png", "restored.png"] : controlMode ? ["initial.png", "fit.png", "remounted.png", "controlled.png"] : ["initial.png", "remounted.png"], sourceHashes: after, sourceHashesStable: true, providerCalls: false, installedServicesTouched: false };
} catch (error) {
  ...
  await runtime.dispose().catch(() => {}); server.stop(true); creationDb.close(); await rm(isolated, { recursive: true, force: true });
}
console.log(JSON.stringify({ passed: true, requests: requests.length, captures: createMode ? 6 : controlMode ? 4 : 2, result: join(output, "result.json") }));
