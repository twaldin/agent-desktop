...
const controlMode = process.argv.includes("--controls");
const sourceFiles = [
...
const server = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(request) {
  ...
  if (url.pathname.startsWith("/v1/")) {
    ...
    const response = await metadataEndpoint.route(request, url) ?? await frameEndpoint.route(request, url) ?? await controlEndpoint.route(request, url) ?? new Response("Not found", { status: 404 });
...
try {
  ...
  await writeFile(join(output, "main.cjs"), `const{app,BrowserWindow,ipcMain}=require('electron'),fs=require('node:fs'),path=require('node:path');const{requestBrowserMetadata}=require('./transport/browser-metadata-transport.cjs'),{requestBrowserFrame}=require('./transport/browser-frame-transport.cjs'),{requestBrowserControl}=require('./transport/browser-control-transport.cjs');
...
ipcMain.handle('native-preview:metadata',(_event,sessionId,hostId)=>{if(hostId!==endpoint.hostId)throw new Error('Unexpected fixture owner');return requestBrowserMetadata(endpoint,sessionId)});ipcMain.handle('native-preview:frame',(_event,sessionId,target,hostId)=>{if(hostId!==endpoint.hostId)throw new Error('Unexpected fixture owner');return requestBrowserFrame(endpoint,sessionId,target)});ipcMain.handle('native-preview:insert-text',(_event,text)=>{if(typeof text!=='string'||text.length>100)throw new Error('Invalid fixture text');return window.webContents.insertText(text)});ipcMain.handle('native-preview:key',(_event,key)=>{if(key!=='End')throw new Error('Invalid fixture key');window.webContents.sendInputEvent({type:'keyDown',keyCode:key});window.webContents.sendInputEvent({type:'keyUp',keyCode:key})});ipcMain.handle('native-preview:control',(_event,sessionId,request,hostId)=>{if(hostId!==endpoint.hostId)throw new Error('Unexpected fixture owner');return requestBrowserControl(endpoint,sessionId,request)});ipcMain.handle('native-preview:capture',async(_event,label)=>{if(!['initial','remounted','controlled'].includes(label))throw new Error('Invalid capture label');fs.writeFileSync(path.join(output,label+'.png'),(await window.webContents.capturePage()).toPNG());return label+'.png'});
app.whenReady().then(async()=>{window=new BrowserWindow({show:false,width:900,height:700,webPreferences:{preload:path.join(output,'preload.cjs'),sandbox:true,contextIsolation:true,nodeIntegration:false,backgroundThrottling:false}});try{await window.loadFile(path.join(output,'web/index.html'),{query:${JSON.stringify({ controls: String(controlMode), hostId, sessionId: session.id, workerPid: String(target.workerPid), name: target.name, targetId: target.targetId, url: tab.url, title: tab.title ?? "" })}});const value=await window.webContents.executeJavaScript('runNativeBrowserPreviewAcceptance()');fs.writeFileSync(path.join(output,'renderer-result.json'),JSON.stringify({...value,electron:process.versions.electron,hidden:true},null,2));window.destroy();app.exit(value.passed?0:1)}catch(error){const progress=await window.webContents.executeJavaScript('nativeBrowserPreviewProgress()').catch(()=>null);fs.writeFileSync(path.join(output,'renderer-result.json'),JSON.stringify({passed:false,error:String(error),progress},null,2));fs.writeFileSync(path.join(output,'failure.png'),(await window.webContents.capturePage()).toPNG());app.exit(1)}});`, { mode: 0o600 });
  ...
  result = { passed: true, scope: "Actual isolated OMP worker and tab through authenticated Bun HTTP, production Electron main transports, minimal preload, and production BrowserPanel", renderer,
    ...
    http: { requestCount: requests.length, requests }, captures: controlMode ? ["initial.png", "remounted.png", "controlled.png"] : ["initial.png", "remounted.png"], sourceHashes: after, sourceHashesStable: true, providerCalls: false, installedServicesTouched: false };
} catch (error) {
...
console.log(JSON.stringify({ passed: true, requests: requests.length, captures: controlMode ? 3 : 2, result: join(output, "result.json") }));
