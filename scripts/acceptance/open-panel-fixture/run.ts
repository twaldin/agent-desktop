...
if (mcp) {
  const ui = await Bun.build({ entrypoints: [join(import.meta.dir, 'mcp-view.ts')], target: 'browser', format: 'esm' });
...
try {
  ...
  await writeFile(join(fixture, 'context.json'), JSON.stringify({ projectId: project.id, sessionId: session.id, git, terminal: !!terminalBundle, mcp }));
