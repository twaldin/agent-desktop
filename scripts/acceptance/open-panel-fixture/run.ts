...
try {
  ...
  await writeFile(join(fixture, 'context.json'), JSON.stringify({ projectId: project.id, sessionId: session.id, git }));
  ...
  await writeFile(join(output, 'preload.cjs'), `const {contextBridge,ipcRenderer}=require('electron');contextBridge.exposeInMainWorld('panelFixture',{call:(method,args)=>ipcRenderer.invoke('panel-call',method,args),subscribe:listener=>{const handler=(_event,value)=>listener(value);ipcRenderer.on('panel-event',handler);return()=>ipcRenderer.removeListener('panel-event',handler);}});`);
