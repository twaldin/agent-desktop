...
app.whenReady().then(async()=>{
 ...
 try {
  ...
  const archived=async visible=>{await options('Chat');await choice(visible?'Show archived chats':'Show active chats')};
  ...
  await click(null,await item('Work older'));check((await invoke('bootstrap')).view.route.sessionId,'Navigation not persisted');await shot('05-list-navigation');await click(null,'.organized-session.selected [aria-label="Pin chat"]');await wait(`!!document.querySelector('section[aria-label="Pinned"] .session-row')`);await shot('05b-pinned-selected');await click(null,'[aria-label="Pinned options"]');await choice('Last updated');await reopen();await click(null,'[aria-label="Pinned options"]');const pinnedPreferences=await invoke('preferencesV2',[(await invoke('bootstrap')).groups[0].state.host.id]);check(pinnedPreferences.records.some(record=>record.key==='sidebar.pinnedSort'&&!record.deleted&&record.value==='updated_at'),'Pinned sorting was not saved to the v2 host record');check(await run(`[...document.querySelectorAll('[aria-label="Pinned options"] [role=menuitemradio]')].some(row=>row.textContent.includes('Last updated')&&row.getAttribute('aria-checked')==='true')`),'Pinned sorting did not survive reopen');await key('Escape');await click(null,'section[aria-label="Pinned"] [aria-label="Unpin chat"]');await wait(`!document.querySelector('section[aria-label="Pinned"]')`);
