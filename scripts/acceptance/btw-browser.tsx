...
const bridge: Partial<DesktopBridge> = {
  ...
  workspaceQuery: (target, query) => request<WorkspaceQueryResult>("/v1/workspace/query", { target, query }), command: (envelope: CommandEnvelope) => request<CommandResult>("/v5/commands", envelope),
...
const state = () => { const field = document.querySelector<HTMLTextAreaElement>('[aria-label="Side chat prompt"]'), fieldStyle = field ? getComputedStyle(field) : undefined; return ({ renderer: { viewport: { width: innerWidth, height: innerHeight }, devicePixelRatio, zoom: visualViewport?.scale ?? 1,
  ...
  prompt: field?.value, promptFocus: fieldStyle ? { active: field === document.activeElement, outlineStyle: fieldStyle.outlineStyle, outlineWidth: fieldStyle.outlineWidth, boxShadow: fieldStyle.boxShadow } : null,
