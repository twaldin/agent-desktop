/** Retained WorkerCore acceptance using its complete selected body and actual channel; controlled dependencies only. */
import assert from "node:assert/strict";
import { readFile } from "node:fs/promises";
import path from "node:path";
import { createHash } from "node:crypto";
const [workerPath, channelPath] = process.argv.slice(2);
if (!workerPath || !channelPath) throw new Error("Usage: browser-retained-evaluation.ts <tab-worker.ts> <worker-cdp-channel.ts>");
const protocolPath = path.join(path.dirname(workerPath), "tab-protocol.ts");
...
function harness(hold?: "loader" | "connect" | "create" | "goto" | "page" | "title" | "init-cleanup") {
  ...
  const dialogPolicies: string[] = [], dialogOutcomes: string[] = []; let dialogHandler: ((dialog: any) => unknown) | undefined; let pageTargetId = "original-page"; let holdPage = false, holdTitle = false;
  ...
  const page = {
    target: () => ({ _targetId: pageTargetId }), isClosed: () => false, on: (event: string, handler?: unknown) => { if (event === "dialog" && handler) { dialogPolicies.push("installed"); dialogHandler = handler as (dialog: any) => unknown; } return page; }, off: () => page,
  ...
  return {
    ...
    stats: () => ({ listeners, removed, guards, disconnected, pageCloses, created, runStarted, runCleanup, loaderCalls, dialogPolicies: [...dialogPolicies], dialogOutcomes: [...dialogOutcomes] }),
    triggerDialog() { assert(dialogHandler, "dialog observer must be installed"); return dialogHandler({ type: () => "confirm", message: () => "controlled", accept: async () => { dialogOutcomes.push("accept"); }, dismiss: async () => { dialogOutcomes.push("dismiss"); } }); },
...
console.log(JSON.stringify({ workerPath, channelPath, protocolPath, workerSha256: createHash("sha256").update(workerSource).digest("hex"), channelSha256: createHash("sha256").update(channelSource).digest("hex"), protocolSha256: createHash("sha256").update(protocolSource).digest("hex"), counts: { pass: passed.length, fail: failed.length }, passed, failed, limits: "Complete selected WorkerCore and channel; type/runtime imports removed, injected loader/browser/page/CDP operations. Existing single dispatcher and actual channel classes exercised, no actual Worker/SDK/IPC/socket/native/timeouts. The controlled page invokes the captured accept/dismiss callback but does not run screenshot capture, so screenshot focus behavior remains unexercised. Parent ownership lease and cmux integration remain separate." }, null, 2));
