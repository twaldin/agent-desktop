...
import { BROWSER_METADATA_OWNER_HEADER } from "../../packages/shared/src/protocol";
...
const repository = resolve(import.meta.dir, "../.."), output = resolve(process.argv[2] ?? `.data/browser-navigation-${Date.now()}`);
const sources = [
  "packages/shared/src/browser-history.ts", "packages/shared/src/browser-control.ts", "packages/shared/src/protocol.ts",
  "apps/host/src/browser-history-http.ts", "apps/host/src/browser-history-revision.ts", "apps/host/src/browser-control-http.ts", "apps/host/src/browser-frame-http.ts", "apps/host/src/browser-metadata-http.ts",
  "apps/host/src/omp-workers/protocol.ts", "apps/host/src/omp-workers/runtime.ts", "apps/host/src/omp-workers/entry.ts",
  "apps/desktop/src/main/browser-history-transport.ts", "apps/desktop/src/main/browser-control-transport.ts", "apps/desktop/src/main/browser-frame-transport.ts", "apps/desktop/src/main/browser-metadata-transport.ts",
  "apps/desktop/src/renderer/BrowserPanel.tsx", "apps/desktop/src/renderer/BrowserAddressInput.tsx", "apps/desktop/src/renderer/browser-preview-source.ts", "apps/desktop/src/renderer/browser-address-suggestions.css", "apps/desktop/src/renderer/browser-panel.css",
  ...
  "patches/@oh-my-pi%2Fpi-coding-agent@18.1.10.patch", "scripts/acceptance/browser-navigation-browser.tsx", "scripts/acceptance/browser-navigation.ts", "bun.lock",
];
...
const isolated = await mkdtemp(join(tmpdir(), "browser-navigation-ui-")), agentDir = join(isolated, "agent"), cwd = join(isolated, "project"), profile = join(isolated, "electron");
...
const token = randomBytes(32).toString("hex"), hostId = "browser-navigation-host", sessionId = "browser-navigation-session";
const slow = Promise.withResolvers<void>(), paths: string[] = [], api: Array<{ path: string; status: number }> = [];
...
const server = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(request) {
  ...
  if (url.pathname.startsWith("/v1/")) { if (request.headers.get("Authorization") !== `Bearer ${token}`) return new Response("Unauthorized", { status: 401 }); const response = await metadata.route(request, url) ?? await frame.route(request, url) ?? await history.route(request, url) ?? await control.route(request, url) ?? new Response("Not found", { status: 404 }); api.push({ path: url.pathname, status: response.status }); return response; }
...
try {
  ...
  await writeFile(join(output, "index.html"), `<meta charset="utf-8"><div id="root"></div><script type="module" src=${JSON.stringify(relative(output, join(import.meta.dir, "browser-navigation-browser.tsx")))}></script>`);
  ...
  const built = await Bun.build({ entrypoints: ["browser-history", "browser-control", "browser-frame", "browser-metadata"].map(name => join(repository, `apps/desktop/src/main/${name}-transport.ts`)), outdir: join(output, "transport"), target: "node", format: "cjs", naming: "[name].cjs" });
  if (!built.success) throw new Error(built.logs.join("\n"));
  const endpoint = { origin: `http://127.0.0.1:${server.port}`, hostId, token };
  await writeFile(join(output, "preload.cjs"), `const{contextBridge,ipcRenderer}=require('electron');contextBridge.exposeInMainWorld('browserNavigationBridge',{getBrowserMetadata:(id,host)=>ipcRenderer.invoke('nav:metadata',id,host),getBrowserFrame:(id,target,host)=>ipcRenderer.invoke('nav:frame',id,target,host),getBrowserHistory:(id,input,host)=>ipcRenderer.invoke('nav:history',id,input,host),controlBrowser:(id,input,host)=>ipcRenderer.invoke('nav:control',id,input,host),pointer:(x,y)=>ipcRenderer.invoke('nav:pointer',x,y),openExternal:()=>Promise.resolve()});`);
  await writeFile(join(output, "main.cjs"), `const{app,BrowserWindow,ipcMain}=require('electron'),path=require('node:path'),fs=require('node:fs');const endpoint=${JSON.stringify(endpoint)},output=${JSON.stringify(output)};const{requestBrowserMetadata}=require('./transport/browser-metadata-transport.cjs'),{requestBrowserFrame}=require('./transport/browser-frame-transport.cjs'),{requestBrowserHistory}=require('./transport/browser-history-transport.cjs'),{requestBrowserControl}=require('./transport/browser-control-transport.cjs');app.setPath('userData',${JSON.stringify(profile)});let win;for(const [name,fn] of [['metadata',(id)=>requestBrowserMetadata(endpoint,id)],['frame',(id,value)=>requestBrowserFrame(endpoint,id,value)],['history',(id,value)=>requestBrowserHistory(endpoint,id,value)],['control',(id,value)=>requestBrowserControl(endpoint,id,value)]])ipcMain.handle('nav:'+name,(_event,id,value,host)=>{if((host??value)!==endpoint.hostId)throw new Error('Wrong host');return fn(id,host===undefined?undefined:value)});ipcMain.handle('nav:pointer',(_event,x,y)=>{if(!Number.isSafeInteger(x)||!Number.isSafeInteger(y))throw new Error('Invalid pointer');win.webContents.sendInputEvent({type:'mouseDown',x,y,button:'left',clickCount:1});win.webContents.sendInputEvent({type:'mouseUp',x,y,button:'left',clickCount:1})});app.whenReady().then(async()=>{win=new BrowserWindow({show:false,width:960,height:720,webPreferences:{preload:path.join(output,'preload.cjs'),sandbox:true,contextIsolation:true,nodeIntegration:false,backgroundThrottling:false}});try{await win.loadFile(path.join(output,'web/index.html'),{query:${JSON.stringify({ hostId, sessionId, workerPid: owner.workerPid, name: tab.name, targetId: tab.targetId, origin: `http://127.0.0.1:${server.port}` })}});const value=await win.webContents.executeJavaScript('runBrowserNavigationAcceptance()');fs.writeFileSync(path.join(output,'renderer-result.json'),JSON.stringify({...value,electron:process.versions.electron},null,2));app.exit(value.passed?0:1)}catch(error){fs.writeFileSync(path.join(output,'renderer-result.json'),JSON.stringify({passed:false,error:error&&error.stack||String(error),text:win.webContents.getURL()},null,2));app.exit(1)}});`);
...
finally { slow.resolve(); await writeFile(join(output, "result.json"), `${JSON.stringify(result, null, 2)}\n`, { mode: 0o600 }).catch(() => {}); await history.dispose().catch(() => {}); await owner?.dispose().catch(() => {}); await runtime.dispose().catch(() => {}); server.stop(true); await rm(isolated, { recursive: true, force: true }); }
