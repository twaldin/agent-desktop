...
const sources = [
  "packages/shared/src/btw.ts", "packages/shared/src/protocol.ts",
  ...
  "apps/host/src/btw.ts", "apps/host/src/btw-http.ts", "apps/host/src/server.ts", "apps/host/src/store.ts", "apps/host/src/validation.ts",
...
try {
  ...
  const capability = crypto.randomUUID(), observed: Array<{route:string;method:string;type?:string;id?:string}> = [];
  ...
  proxy = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(request) {
    ...
    observed.push({ route, method: request.method, type: (body as any)?.command?.type, id: (body as any)?.id });
    ...
    const allowedPost = ["/v5/commands","/v1/workspace/query","/v1/models/composer"].includes(route);
    if (!(request.method === "GET" && allowedGet) && !(request.method === "POST" && allowedPost)) return new Response(null, { status: 403, headers: cors });
    const response = await hostRequest(route, body, route.endsWith("/btw") || route.endsWith("/questions")), bytes = await response.arrayBuffer();
  ...
  Object.assign(result, { exitCode, sourceAtBuild: before, sourceAfterBuild: sourceAfter, sourceHashesStable: JSON.stringify(before) === JSON.stringify(sourceAfter),
    hostEvidence: { sessionId: session.id, mainMessagesUnchanged: JSON.stringify(messagesBefore) === JSON.stringify(messagesAfter), mainMessageCount: messagesAfter.length, commands, providerCalls: (await readdir(gates)).filter(name => name.endsWith(".started")).sort() },
  ...
  result.passed &&= exitCode === 0 && result.sourceHashesStable && result.hostEvidence.mainMessagesUnchanged && commands.length === 3 && result.captures?.length === 5 && result.state?.checks?.length === 5;
