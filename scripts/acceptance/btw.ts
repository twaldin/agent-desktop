...
try {
  ...
  result.passed &&= exitCode === 0 && result.sourceHashesStable && result.hostEvidence.mainMessagesUnchanged && commands.length === (slash ? 4 : 3) && result.captures?.length === (slash ? 7 : 5) && result.state?.checks?.length === (slash ? 7 : 5);
