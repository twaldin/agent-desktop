...
await writeFile(join(agentDir,'mcp.json'),JSON.stringify({mcpServers:{fixture:{command:process.execPath,args:[join(repo,'apps/host/src/omp/fixtures/mcp-server.ts')],env:{AGENT_DESKTOP_MCP_TEST_MARKER:marker,AGENT_DESKTOP_MCP_TEST_RESOURCE_DELAY:'400'}}}}));
...
try{
 const before=await hashes(),session=await runtime.create({cwd,interactions:true}),receipts=new Map<string,NativeSessionMcpReceipt>();let reloads=0,receiptReads=0,lostAcknowledgements=0;
 ...
 proxy=Bun.serve({hostname:'127.0.0.1',port:0,async fetch(request){const url=new URL(request.url);if(!url.pathname.startsWith('/'+capability+'/'))return new Response(null,{status:404});if(request.method==='OPTIONS')return new Response(null,{status:204,headers:cors});const route=url.pathname.slice(capability.length+1);
  ...
  if(route==='/reload'&&request.method==='POST'){const envelope=await request.json() as CommandEnvelope;const command=envelope.command;if(command.type!=='session.mcp.reload'||command.sessionId!==session.id||receipts.has(envelope.id))throw new Error('Invalid or duplicate UI dispatch');receipts.set(envelope.id,{commandId:envelope.id,state:'pending'});reloads++;try{const snapshot=await session.reloadSessionMcp({epoch:command.epoch,expectedRevision:command.expectedRevision});receipts.set(envelope.id,{commandId:envelope.id,state:'succeeded'});if(loseAck){lostAcknowledgements++;return new Response('Injected lost acknowledgement after native completion',{status:503,headers:cors});}return Response.json({ok:true,commandId:envelope.id,value:{type:'session.mcp',snapshot}},{headers:cors});}catch{receipts.set(envelope.id,{commandId:envelope.id,state:'failed'});return Response.json({ok:false,commandId:envelope.id,error:{code:'FAILED',message:'Controlled UI reload failed'}},{headers:cors});}}
