...
export default function(pi: ExtensionAPI) {
  const gates = process.env.ARTIFACT_CONTRACT_GATES;
  ...
  pi.registerProvider("artifact-contract", {baseUrl:"https://controlled.invalid", apiKey:"inert-local-fixture-key",api:"artifact-contract-api" as Api,
    ...
    streamSimple(model,context,options) {
      ...
      void (async()=>{
        ...
        const mounted = listed && JSON.stringify(results[0]).includes('mcp__fixture_report');
        const answered = results.length > 1;
        const tool = !listed ? {name:'read',arguments:{path:'xd://'}} : mounted && !answered ? {name:'write',arguments:{path:'xd://mcp__fixture_report',content:'{"title":"Original report"}' }} : undefined;
        writeFileSync(path.join(gates,"tools.json"),JSON.stringify(context.tools?.map(tool=>tool.name)));
        const aborted = options?.signal?.aborted;
        const message:AssistantMessage={role:"assistant",content:aborted?[]:answered?[{type:"text",text:"Original report completed."}]:tool?[{type:"toolCall",id:crypto.randomUUID(),name:tool.name,arguments:tool.arguments}]:[{type:"text",text:"No MCP tools registered."}],
