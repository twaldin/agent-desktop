...
export type WorkerEvent = { type: "tree_changed" } | { type: "plan_changed" } | { type: "todos_changed" } | NativeEventMetadata | OmpBridgeEvent | { type: "queued_messages_changed"; snapshot: NativeQueuedMessagesSnapshot };
...
export function projectWorkerEvent(event: OmpRuntimeEvent): WorkerEvent {
  if (event.type === "plan_changed" || event.type === "todos_changed" || event.type === "tree_changed") return { type: event.type };
