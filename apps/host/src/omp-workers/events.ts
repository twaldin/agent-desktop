...
export type WorkerEvent = { type: "turn_review_changed" } | { type: "tree_changed" } | { type: "plan_changed" } | { type: "todos_changed" } | NativeEventMetadata | OmpBridgeEvent | { type: "queued_messages_changed"; snapshot: NativeQueuedMessagesSnapshot };
...
export function projectWorkerEvent(event: OmpRuntimeEvent): WorkerEvent {
  if (event.type === "turn_review_changed" || event.type === "plan_changed" || event.type === "todos_changed" || event.type === "tree_changed") return { type: event.type };
