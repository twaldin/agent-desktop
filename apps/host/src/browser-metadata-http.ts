import { BROWSER_METADATA_OWNER_HEADER, BROWSER_METADATA_PROTOCOL_VERSION, type BrowserMetadataAvailability } from "@agent-desktop/shared";
...
export class BrowserMetadataHttp {
  constructor(private options: { hostId: string; sessionExists(sessionId: string): boolean; getExistingHandle(sessionId: string): Promise<{ workerFailure?: { message: string }; getBrowserMetadata(): Promise<BrowserMetadataAvailability> } | undefined> }) {}
  ...
  async route(request: Request, url = new URL(request.url)): Promise<Response | undefined> {
    ...
    try {
      ...
      return Response.json({ protocolVersion: BROWSER_METADATA_PROTOCOL_VERSION, hostId: this.options.hostId, sessionId, ...metadata }, { headers });
    } catch (error) {
