...
Create one manager per owning host service. The host resolves the project/session catalog identity to its canonical directory and passes both to `create`; clients do not choose an unrestricted shell executable, environment or absolute root. Production uses the OS account shell when it is an absolute path, then an absolute `SHELL` environment value, with login/interactive arguments. When both are unavailable (including Bun's `"unknown"` account metadata), the fallback is `/bin/zsh` on macOS and `/bin/sh` elsewhere. Shell tests use isolated startup directories and disabled history, or explicitly profile-free Bash configurations. Terminal commands execute with the owning user's normal privileges; the initial directory is ownership context, not a filesystem sandbox.
