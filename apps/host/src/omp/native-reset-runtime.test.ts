...
test("actual OmpRuntime installs the synchronous root factory after enabling its writer and finishes after native disposal", async () => {
  ...
  const events: string[] = [], lifecycle: string[] = []; let binding!: Readonly<CodexResetPolicySessionBinding>, unavailable = false, signalUnavailable = false;
  const runtime = new OmpRuntime({ agentDir: f.agentDir, createResetPolicyOwner: (value, interactions) => {
    ...
    value.registerLifecycle({ beginClose: () => lifecycle.push("beginClose"), drained: () => lifecycle.push("drained") });
  ...
  try {
    ...
    await session.dispose(); expect(events).toEqual(["begin:root", "finish:root"]); expect(lifecycle).toEqual(["beginClose", "drained"]); expect(binding.settings.getResetPolicySettingsWriter()).toBeNull();
