...
interface Entry { id: string; type?: string; message?: unknown }
...
export function recordedGeneratedImage(entry: Entry | undefined, index: number): OmpRecordedImage {
...
export function projectSessionOutputs(entries: readonly Entry[], cwd: string): { outputs: SessionOutput[]; warnings: string[]; truncated: boolean } {
  const warnings = new Set<string>();
  type Generated = { entryId: string; turnId: string; revision: string; label: string; kind: 'generated-image'; path: string; imageIndex: number; entry: Entry };
  type Candidate = Exclude<SessionOutput, { kind: 'generated-image' }> | Generated;
  type Turn = { id: string; outputs: Candidate[]; reads: Set<string>; final?: { entry: Entry; text: string }; complete: boolean };
  ...
  for (const entry of entries.slice(-5000)) {
    ...
    if (m.role === 'user') { turn = { id: entry.id, outputs: [], reads: new Set(), complete: false }; turns.push(turn); calls.clear(); }
    if (!turn) continue;
  ...
  for (const turn of turns.reverse()) {
    if (turn.complete && turn.final) {
      ...
      if (!turn.outputs.some(o => o.kind === 'file' || o.kind === 'mcp') && website) turn.outputs.push({ kind: 'website', url: website, label: website.slice(0, 1000), entryId: turn.final.entry.id, turnId: turn.id, revision: hash([turn.final.entry.id, website]) });
    }
...
export async function inspectSessionOutputs(entries: readonly Entry[], cwd: string, epoch: string): Promise<SessionOutputs> {
  const projected = projectSessionOutputs(entries, cwd), outputs: SessionOutput[] = [];
  for (const output of projected.outputs) {
    if (output.kind !== 'file' && output.kind !== 'generated-image') { outputs.push(output); continue; }
    try {
