import { BTW_OWNER_HEADER, BTW_PROTOCOL_VERSION, type NativeBtwSnapshot } from "@agent-desktop/shared";
...
export class BtwHttp {
  ...
  async route(request: Request, url = new URL(request.url)): Promise<Response | undefined> {
    ...
    try {
      ...
      return Response.json({ protocolVersion: BTW_PROTOCOL_VERSION, hostId: this.options.hostId, sessionId, value } satisfies { protocolVersion: 1; hostId: string; sessionId: string; value: NativeBtwSnapshot | null }, { headers });
