...
export function parseWindowView(value: unknown): WindowViewState | undefined {
  ...
  if (value.environmentOpen !== undefined && typeof value.environmentOpen !== "boolean") return;
  ...
  return { route: { sessionId: route.sessionId as string | null, ...(route.hostId === undefined ? {} : { hostId: route.hostId as string }) },
    ...(dock ? { dock } : {}),
    ...(typeof value.environmentOpen === "boolean" ? { environmentOpen: value.environmentOpen } : {}),
...
export function parseDockSnapshot(value: unknown): WindowViewState["dock"] {
  ...
  const region = (raw: unknown): DockState["right"] | undefined => {
    if (!record(raw) || typeof raw.open !== "boolean" || !Array.isArray(raw.tabIds) || raw.tabIds.length > 100) return;
    const ids: string[] = [];
    for (const tabId of raw.tabIds) { if (typeof tabId !== "string" || !seen.has(tabId) || used.has(tabId)) return; used.add(tabId); ids.push(tabId); }
    if (raw.activeTabId !== undefined && (typeof raw.activeTabId !== "string" || !ids.includes(raw.activeTabId))) return;
    return { tabIds: ids, open: raw.open, ...(raw.activeTabId === undefined ? {} : { activeTabId: raw.activeTabId as string }) };
  };
  const right = region(value.state.right), bottom = region(value.state.bottom);
  const ratio = value.state.rightWidthRatio, height = value.state.bottomHeight;
  if (!right || !bottom || typeof ratio !== "number" || !Number.isFinite(ratio) || ratio < 0 || ratio > 1 || typeof height !== "number" || !Number.isFinite(height) || height < 0 || height > 4096 || used.size !== tabs.length) return;
  return { tabs, state: { right, bottom, rightWidthRatio: ratio, bottomHeight: height } };
}
