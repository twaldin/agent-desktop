import { parseBranchQueryRequest, type BranchQueryObserverStatus, type BranchQueryRequest, type BranchQueryObserverView } from "@agent-desktop/shared";
import type { BranchQueryConnection } from "./branch-query-connection";

interface Entry {
  request: BranchQueryRequest;
  view: BranchQueryObserverView;
  isCurrent(): boolean;
  released: boolean;
  pending?: Promise<void>;
  lease?: ReturnType<BranchQueryConnection["retain"]>;
}
...
export class BranchQueryWindows {
  #windows = new Map<number, WindowOwner>();
  #disposed = false;
  constructor(private options: {
    connect(hostId: string, isCurrent: () => boolean): Promise<BranchQueryConnection>;
    notify(senderId: number, status: BranchQueryObserverStatus): void;
  }) {}
  ...
  #publish(senderId: number, owner: WindowOwner, entry: Entry): void {
    ...
    try { this.options.notify(senderId, { hostId: entry.request.hostId, subscriptionId: entry.request.subscriptionId, target: { ...entry.request.target }, query: { ...entry.request.query }, view: structuredClone(entry.view) }); }
    catch (error) { console.error("Branch query window delivery failed:", error); this.releaseWindow(senderId); }
  }
  dispatch(senderId: number, value: unknown, isCurrent: () => boolean, token: unknown): Promise<void> {
    const owner = this.#windows.get(senderId);
    if (this.#disposed || !owner || token !== owner.token || !isCurrent()) throw new Error("The branch query window is unavailable.");
    const request = parseBranchQueryRequest(value);
    let entry = owner.entries.get(request.subscriptionId);
    if (entry && (entry.request.hostId !== request.hostId || JSON.stringify(entry.request.target) !== JSON.stringify(request.target) || JSON.stringify(entry.request.query) !== JSON.stringify(request.query))) {
      throw new Error("The branch query observer belongs to a different owner.");
    }
    if (!entry) {
      entry = { request, view: { phase: "unavailable" }, isCurrent, released: false };
      if (request.action === "inspect" || request.action === "recover") { this.#publish(senderId, owner, entry); return Promise.resolve(); }
      if (owner.entries.size >= 2048) throw new Error("The branch query window history is full. Reopen this window to continue.");
      ...
      if (request.action === "retain") {
        ...
        if (active >= 128) entry.view = { phase: "failed", error: "Too many branch queries in this window." };
    ...
    else if (request.action === "recover" && this.#current(senderId, owner, entry)) entry.lease?.recover();
  ...
  #start(senderId: number, owner: WindowOwner, entry: Entry): void {
    ...
    entry.pending = Promise.resolve().then(async () => {
      ...
      if (!current()) return;
      const lease = connection.retain(entry.request.target, entry.request.query, view => {
        if (!current()) return;
