...
import type { BranchQueryObserverStatus, BranchQueryRequest } from "@agent-desktop/shared";
...
const request = (action: BranchQueryRequest["action"] = "retain", subscriptionId = "one", hostId = "host", projectId = "project"): BranchQueryRequest => ({ type: "branch-query", version: 1, action, subscriptionId, hostId, target: { projectId }, query });
function fixture() {
  const sent: BranchQueryRequest[] = [], statuses: { senderId: number; status: BranchQueryObserverStatus }[] = [], calls: string[] = [];
  let connect: (hostId: string, isCurrent: () => boolean) => Promise<BranchQueryConnection>;
  let notify: ((senderId: number, status: BranchQueryObserverStatus) => void) | undefined;
  const client = new BranchQueryConnection("host");
  const channel = client.attach({ send: value => sent.push(value), close: () => {} });
  channel.receive({ type: "state", replayComplete: true, state: { host: { id: "host" }, branchQueries: { version: 1 } } });
  connect = async () => client;
  const windows = new BranchQueryWindows({ connect: (hostId, current) => { calls.push(hostId); return connect(hostId, current); },
    notify: (senderId, status) => { statuses.push({ senderId, status }); notify?.(senderId, status); } });
  windows.reset(1); windows.reset(2);
  cleanups.push(() => { windows.dispose(); client.dispose(); });
...
test("same observer duplicates share admission, while inspection only uses an existing lease", async () => {
  const f = fixture(), gate = deferred<BranchQueryConnection>(); f.connect = () => gate.promise;
...
test("navigation reset releases current wires and old replies do not reach the new renderer lifetime", async () => {
  ...
  f.channel.receive({ type: "branch-query", version: 1, hostId: "host", subscriptionId: old.subscriptionId, target: old.target, query: old.query, event: "status", phase: "ready" });
  expect(f.statuses).toHaveLength(count);
  f.channel.receive({ type: "branch-query", version: 1, hostId: "host", subscriptionId: next.subscriptionId, target: next.target, query: next.query, event: "status", phase: "ready" });
...
test("the same logical ID in another window owns an independent lease and delivery destination", async () => {
  ...
  f.channel.receive({ type: "branch-query", version: 1, hostId: "host", subscriptionId: second.subscriptionId, target: second.target, query: second.query, event: "status", phase: "ready", error: "partial" });
...
test("host/target mismatch and malformed inputs cannot redirect an existing observer", async () => {
  ...
  for (const invalid of [request("release", "one", "other"), request("retain", "one", "host", "other"), { ...request(), query: { type: "git.default-branch" } }, { ...request(), target: { filePath: "/repo" } }, { ...request(), version: 2 }]) {
...
test("observed frame loss latches across return, and post-lookup frame loss suppresses admission", async () => {
  for (const restore of [false, true]) {
    const f = fixture(), gate = deferred<BranchQueryConnection>(); let current = true, admitted: (() => boolean) | undefined;
...
test("navigation denies outgoing document requests before and after the new document loads", async () => {
  ...
  try {
    ...
    for (const action of ["retain", "inspect", "release", "recover"] as const) {
...
test("unknown recovery cannot look up a host; ready recovery preserves exact logical query", async () => {
  const f = fixture();
  await f.windows.dispatch(1, request("recover"), () => true, f.windows.token(1));
  expect(f.calls).toEqual([]); expect(f.statuses.at(-1)?.status.view.phase).toBe("unavailable");
  await f.windows.dispatch(1, request(), () => true, f.windows.token(1));
  const { action: _, ...wire } = f.sent[0]!;
  f.channel.receive({ ...wire, event: "status", phase: "ready" });
  await f.windows.dispatch(1, request("recover"), () => true, f.windows.token(1));
  expect(f.sent.map(r => r.action)).toEqual(["retain", "recover"]);
  expect(f.sent.at(-1)).toMatchObject({ target: { projectId: "project" }, query });
});

test("renderer mutation of nested result cannot corrupt repeated publication", async () => {
  const f = fixture(); await f.windows.dispatch(1, request(), () => true, f.windows.token(1));
  const { action: _, ...wire } = f.sent[0]!;
  f.channel.receive({ ...wire, event: "status", phase: "ready" });
  f.notify = (_id, status) => {
    if (status.view.phase === "ready" && status.view.update?.phase === "complete" && status.view.update.result.type === "git.recent-branches") status.view.update.result.branches.push("renderer edit");
    Object.assign(status.query, { limit: 1 });
  };
  f.channel.receive({ ...wire, event: "result", update: { generation: 1, requiresRecovery: false, phase: "complete", result: { type: query.type, branches: ["main"] } } });
  f.notify = undefined;
  await f.windows.dispatch(1, request("inspect"), () => true, f.windows.token(1));
  expect(f.statuses.at(-1)?.status).toMatchObject({ query, view: { update: { result: { branches: ["main"] } } } });
});
