...
export async function captureDesktop(window: BrowserWindow, destination: string): Promise<void> {
  ...
  const read = (view: View): Promise<Readiness> => script(`
    ...
    const missingTerminal = view === 'terminal' && visible(scope) && !scope.querySelector('.dock-native-terminal') && visible(scope.querySelector('[data-dock-empty-actions]'));
    ...
    for (const view of selectedViews as View[]) {
      ...
      try {
        ...
        if (["files", "git", "worktrees", "terminal"].includes(view)) {
          ...
          else await script(`if (!visible(document.querySelector('.workspace-panel'))) { const side = document.querySelector('[aria-label="Toggle side panel"][aria-checked="false"]'); if (side) { side.click(); await settle(); } click('.dock-slot-right [data-dock-empty-actions] button', 'Files'); await settle(); } click('#workspace-tab-${view === "git" ? "changes" : view}'); await settle();`);
        }
