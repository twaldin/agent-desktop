...
export function NativePluginSummary({plugin,disabled,onToggle}:{plugin:NativePlugin;disabled:boolean;onToggle():void}) {
 return <div className="native-plugin-summary">
  ...
  <div className="integration-title plugin-detail-title">
   ...
   {plugin.canToggle && <button type="button" className={`plugin-detail-enable ${plugin.enabled?'secondary-button':'primary-button'}`} aria-label={`${plugin.enabled?'Disable':'Enable'} ${plugin.title || plugin.name}`} aria-pressed={plugin.enabled} disabled={disabled} onClick={onToggle}>{plugin.enabled?<><Icon name="check"/>Enabled</>:'Enable'}</button>}
  </div>
