...
export function ComposerSelectionPopup({ modelValue, modelLabel, modelTitle, models, levels, effort, effectiveEffort, defaultEffortLabel, disabled, onModel, onEffort, onReset }: {
  ...
  useEffect(() => {
    ...
    const outside = (event: PointerEvent) => { if (!root.current?.contains(event.target as Node)) { setOpen(undefined); setQuery(""); } };
  ...
  return <div className="composer-selection-popup" ref={root}>
    ...
    {open && <div ref={menu} className="composer-selection-menu" role="menu" aria-label={open === "models" ? "Select model" : open === "effort" ? "Select effort" : "Composer selections"} onKeyDown={moveMenuFocus}>
      {open === "main" && <>
