...
export function ComposerSelectionPopup({ modelValue, modelLabel, modelTitle, models, levels, effort, effectiveEffort, defaultEffortLabel, disabled, onModel, onEffort, onReset }: {
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      const box = anchor.getBoundingClientRect(), width = Math.min(330, innerWidth - 24);
  ...
  const label = `${modelLabel}${activeEffort ? ` ${activeEffort}` : ""}`;
  ...
  return <div className="composer-selection-popup" ref={root}>
    <button ref={trigger} type="button" className="composer-selection-trigger" aria-label="Model and reasoning effort" aria-haspopup="menu" aria-expanded={Boolean(open)} title={modelTitle} disabled={disabled} onKeyDown={event => { if (event.key === "Escape" && open) { event.preventDefault(); close(); } }} onClick={() => { if (disabled) return; setOpen(open ? undefined : "main"); }}><Icon name="sliders"/><span>{label}</span><Icon name="chevron"/></button>
    {open && position && createPortal(<div style={position} ref={menu} className="composer-selection-menu" role="menu" aria-label={open === "models" ? "Select model" : open === "effort" ? "Select effort" : "Composer selections"} onKeyDown={moveMenuFocus}>
      {open === "main" && <>
        <div className="composer-selection-header"><span><b>{activeEffort ?? defaultEffortLabel}</b><small>{selected?.label ?? modelLabel}</small></span>{(modelValue || effort) && <button type="button" aria-label="Reset composer selections" onClick={() => { if (disabled) return; onReset(); close(); }}><Icon name="refresh"/></button>}</div>
        <button role="menuitem" type="button" onClick={() => { if (!disabled) setOpen("models"); }}>Model <span>{selected?.label ?? modelLabel}</span><Icon name="chevron"/></button>
        {ordinal.length > 0 && <label className="composer-selection-power"><span>Power</span><input aria-label="Reasoning power" type="range" min="0" max={ordinal.length - 1} step="1" value={Math.max(0, activeOrdinal)} disabled={disabled || activeOrdinal < 0} onChange={event => { if (!disabled) onEffort(ordinal[Number(event.target.value)]); }}/><output>{activeOrdinal >= 0 ? ordinal[activeOrdinal] : "Select effort"}</output></label>}
        {levels.filter(level => level === "auto" || level === "off").map(level => <button role="menuitemradio" aria-checked={activeEffort === level} key={level} type="button" onClick={() => { if (disabled) return; onEffort(level); close(); }}>{level}{activeEffort === level && <Icon name="check"/>}</button>)}
        {levels.length > 0 && <button role="menuitem" type="button" onClick={() => { if (!disabled) setOpen("effort"); }}>Select effort <Icon name="chevron"/></button>}
      </>}
