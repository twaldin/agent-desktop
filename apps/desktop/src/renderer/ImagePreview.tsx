...
export function ImagePreview({ media, source, hostId, connected, label, className = "" }: ImagePreviewProps) {
  const key = `${hostId}:${attachmentSourceKey(source)}`;
  const [state, setState] = useState<{ key: string; url?: string; error?: string }>({ key });
  const [retry, setRetry] = useState(0), [expanded, setExpanded] = useState(false);
  ...
  const failure = () => { setState({ key, error: "This device could not display the image." }); setExpanded(false); };
  return <div className={`image-preview ${className}`}>
    {current?.url ? <button type="button" className="image-preview-thumbnail" aria-label={`Preview ${label}`} onClick={() => setExpanded(true)}><img src={current.url} alt={label} onError={failure}/></button>
    ...
    <dialog ref={dialog} className="image-preview-dialog" aria-label={`Image preview: ${label}`} onCancel={() => setExpanded(false)} onClick={event => { if (event.target === event.currentTarget) setExpanded(false); }}>
      <div className="image-preview-heading"><span>{label}</span><button type="button" aria-label="Close image preview" onClick={() => setExpanded(false)}>Close</button></div>
