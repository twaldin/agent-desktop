  ...
  const bridge = {
    workspaceQuery: async (_target: unknown, query: Parameters<ConstructorParameters<typeof WorkspaceState>[0]["workspaceQuery"]>[1], hostId?: string): Promise<WorkspaceQueryResult> => {
      ...
      switch (query.type) {
        ...
        case "git.file-inspect": case "git.file-history": case "git.file-revision": throw new Error("Git file history is exercised through the dedicated original-owner history fixtures.");
