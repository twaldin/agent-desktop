...
async function fixture() {
  ...
  const bridge = {
    workspaceQuery: async (_target: unknown, query: Parameters<ConstructorParameters<typeof WorkspaceState>[0]["workspaceQuery"]>[1], hostId?: string): Promise<WorkspaceQueryResult> => {
      owners.push(hostId!);
      switch (query.type) {
        case "environment.output": case "environment.preparation": case "environment.read": case "environments.list": throw new Error("Environment catalog is outside this file/Git fixture.");
    ...
    command: async (envelope: CommandEnvelope, hostId?: string): Promise<CommandResult> => {
      ...
      try {
        ...
        switch (action.type) {
          case "environment.save": throw new Error("Environment editing is outside this file/Git fixture.");
