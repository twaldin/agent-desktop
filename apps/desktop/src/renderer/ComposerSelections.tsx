...
export function ComposerSelections({ data, draft, session, disabled, onChange }: {
  ...
  return <>
    ...
    {(selection.reasoning || draft.thinkingLevel) && <label className="select-control reasoning-select" title={selection.effectiveDefaultThinking ? `Native default resolves to ${selection.effectiveDefaultThinking}${selection.defaultThinking === "auto" ? " until automatic turn selection" : ""}` : thinkingLabel}><span className="sr-only">Reasoning effort</span><select aria-label="Reasoning effort" value={draft.thinkingLevel ?? ""} disabled={disabled} onChange={event => onChange({ thinkingLevel: event.target.value || undefined })}><option value="">{thinkingLabel}</option>{selection.levels.map(level => <option key={level} value={level}>{level}</option>)}{draft.thinkingLevel && !selection.levels.includes(draft.thinkingLevel) && <option value={draft.thinkingLevel}>{draft.thinkingLevel} (saved)</option>}</select></label>}
  </>;
}
