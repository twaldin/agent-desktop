...
/** A dismissal consumes only the still-current captured query, not a newer draft. */
export function forkSlashQueryRemoval(query: { text: string; start: number; end: number }, currentText: string, selection: { start: number; end: number }) {
  if (currentText !== query.text) return;
  const map = (position: number) => Math.min(position, query.start) + Math.max(0, position - query.end);
  return { text: currentText.slice(0, query.start) + currentText.slice(query.end), selection: { start: map(selection.start), end: map(selection.end) } };
}

/** Pinned bHr orders local first. Its visibility input is source Git capability, not destination eligibility. */
export function sessionForkDestinations(snapshot: SessionForkSnapshot): SessionForkDestination[] {
  const sameWorktree = snapshot.local.isWorktree;
...
export interface SessionForkMenuRequest { anchor: HTMLElement; restoreFocus(): void; dismissQuery?(): void }
