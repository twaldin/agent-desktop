...
export function PluginAcquisition({bridge,hostId,target,connected,visible,query,actionsRoot,onMcp,onInstalledChanged,children}:Props){
 ...
 const start=async(action:NativePluginAcquisition)=>{
  ...
  const ticket=epoch.current;const id=crypto.randomUUID();
  ...
  try{
   ...
   if(action.operation==='marketplace.add')setSource(current=>current.trim()===action.source?'':current);closeDialog();
 ...
 {dialogOpen&&createPortal(<dialog ref={dialog} className="marketplace-dialog" aria-labelledby="marketplace-dialog-title" onCancel={event=>{event.preventDefault();closeDialog();}} onClick={event=>{if(event.target===event.currentTarget)closeDialog();}}>
