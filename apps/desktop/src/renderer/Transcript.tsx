...
export function TranscriptMessages({ messages, contextKey, connected, linkActions }: { messages: TranscriptMessage[]; contextKey: string; connected: boolean; linkActions?: TranscriptLinkActions }) {
  ...
  return <TranscriptMarkdownContext value={{ actions: linkActions, views: markdownViews }}>{messages.map(message => <TranscriptItem key={message.id} message={message} connected={connected} disclosures={disclosures} calls={links.calls} linkedCall={links.results.get(message.id)}/>)}</TranscriptMarkdownContext>;
}
export function TranscriptItem({ message, connected, disclosures, calls, linkedCall }: { message: TranscriptMessage; connected: boolean; disclosures: TranscriptDisclosureState; calls: Map<string, ToolLink>; linkedCall?: ToolLink }) {
  ...
  if (message.role === "toolResult" || message.role === "tool") {
    ...
    return <div className="transcript-tool-result" data-message-id={message.id} data-native-id={message.nativeId}>
      <Disclosure state={disclosures} stateKey={message.id} running={outcome.tone === "running"} label={`${name} result`} tone={outcome.tone} status={outcome.label}>
        ...
        {blocks.length ? blocks.map((block, index) => <Block key={index} block={block} blockKey={`${message.id}:block:${index}`} disclosures={disclosures} calls={calls} connected={connected} toolOutput/>) : <p className="transcript-empty-output">{message.tool?.isError ? "The tool failed without output." : message.tool?.status === "completed" ? "No output was returned." : "No output has been received."}</p>}
      </Disclosure>
    </div>;
  }
  const user = message.role === "user", assistant = message.role === "assistant";
  if (!user && !assistant) return <section className="transcript-native-message" data-message-id={message.id} aria-label={`Native ${message.role} message`}><div className="transcript-native-role">{message.role}</div>{blocks.map((block, index) => <Block key={index} block={block} blockKey={`${message.id}:block:${index}`} disclosures={disclosures} calls={calls} connected={connected}/>)}{!blocks.length && <p className="subtle-notice">No displayable content was supplied for this native message.</p>}</section>;
  ...
  return <article className={`message ${user ? "user-message" : "assistant-message"}`} data-message-id={message.id} data-native-id={message.nativeId} aria-label={user ? "Your message" : "Assistant message"}>
    <div className="message-body">{blocks.map((block, index) => <Block key={index} block={block} blockKey={`${message.id}:block:${index}`} disclosures={disclosures} calls={calls} connected={connected}/>)}
...
function Block({ block, blockKey, disclosures, calls, connected, toolOutput = false }: { block: TranscriptBlock; blockKey: string; disclosures: TranscriptDisclosureState; calls: Map<string, ToolLink>; connected: boolean; toolOutput?: boolean }) {
