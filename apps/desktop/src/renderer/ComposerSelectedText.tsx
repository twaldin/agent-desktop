...
export function ComposerSelectedText({ attachments, disabled = false, onRemove, onFocusComposer }: {
  ...
  const keepOpen = () => { clearCloseTimer(); setOpen(true); };
  const close = (restoreFocus = false) => {
    clearCloseTimer();
    setOpen(false);
    if (restoreFocus) onFocusComposer?.();
  };

  useEffect(() => () => clearCloseTimer(), []);
  useEffect(() => { if (!selected.length) close(); }, [selected.length]);
  useLayoutEffect(() => {
    if (!open || !trigger.current || !preview.current) return;
    const update = () => {
      const anchor = trigger.current?.getBoundingClientRect();
      const content = preview.current?.getBoundingClientRect();
      if (!anchor || !content) return;
      const above = anchor.top - 4 - COLLISION.top;
      const below = innerHeight - anchor.bottom - 4 - COLLISION.bottom;
      const top = above >= Math.min(content.height, below) || above >= below;
      const maxHeight = Math.max(0, top ? above : below);
      setPosition({
        left: Math.max(COLLISION.left, Math.min(anchor.left, innerWidth - content.width - COLLISION.right)),
        ...(top ? { bottom: innerHeight - anchor.top + 4 } : { top: anchor.bottom + 4 }),
        maxHeight: Math.min(14 * 16, maxHeight),
      });
    };
    update();
    ...
    const outsideFocus = (event: FocusEvent) => {
      ...
      if (!trigger.current?.contains(next) && !preview.current?.contains(next)) close();
    };
    addEventListener("focusin", outsideFocus);
    addEventListener("pointerdown", outsideFocus);
    return () => {
      removeEventListener("resize", update);
      removeEventListener("scroll", update, true);
      removeEventListener("focusin", outsideFocus);
      removeEventListener("pointerdown", outsideFocus);
    };
  }, [open]);
  ...
  const popup = open && typeof document !== "undefined" && createPortal(<div ref={preview} id={previewId} role="dialog" aria-label="Selected text" className="composer-selected-text-preview" style={position}
    onMouseEnter={keepOpen} onMouseLeave={closeLater} onKeyDown={event => { event.stopPropagation(); if (event.key === "Escape") { event.preventDefault(); close(true); } }}>
    {selected.map(attachment => <span className="composer-selected-text-snippet" key={attachment.id}>“{attachment.text}”</span>)}
  </div>, document.body);

  return <span className="composer-selected-text" onMouseEnter={keepOpen} onMouseLeave={closeLater}>
    <button ref={trigger} type="button" className="composer-selected-text-chip" aria-expanded={open} aria-haspopup="dialog" aria-controls={previewId}
