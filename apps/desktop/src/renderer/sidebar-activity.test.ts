...
test("activity includes unread/running/waiting local chats but not read, archived or another host's waiting state", () => {
  const session = (hostId: string, id: string, status: SessionSummary["status"] = "idle", archived = false): SessionSummary => ({ hostId, id, status, archived, title: id, createdAt: 1, updatedAt: 2, cwd: "/fixture", sessionFile: "/fixture/chat.jsonl" });
