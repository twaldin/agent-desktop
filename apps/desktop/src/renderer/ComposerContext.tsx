...
import type { LocalEnvironmentCatalogItem, LocalEnvironmentSelection, NewChatExecution, Project } from "@agent-desktop/shared";
...
export const ComposerContext = forwardRef<ComposerContextHandle, {
  ...
  environment?: LocalEnvironmentSelection; environments?: { items: LocalEnvironmentCatalogItem[]; loading: boolean; error?: string; refresh(): void };
  ...
  return <div className="composer-context" ref={root}>
    ...
    {open && open !== "create-branch" && position && createPortal(<div ref={menu} role="menu" aria-label={label} className="composer-context-menu" style={position} onKeyDown={key}>
      ...
      {open === "environments" && <><p className="context-menu-heading">Environment</p>{environments?.loading && <p role="status">Loading environments…</p>}{environments?.error && <p role="alert">{environments.error}<button type="button" onClick={() => environments.refresh()}>Retry</button></p>}{!environments?.loading && !environments?.error && <div className="context-options"><button role="menuitemradio" aria-checked={environment === null} type="button" onClick={() => { onEnvironment?.(null); close(); }}><Icon name="close"/><span>Work without environment</span>{environment === null && <Icon name="check"/>}</button>{environments?.items.map(item => item.type === "environment" ? <button key={item.configPath} role="menuitemradio" aria-checked={environment?.configPath === item.configPath && environment.revision === item.revision} type="button" onClick={() => { onEnvironment?.({ projectId: projectId!, configPath:item.configPath, revision:item.revision }); close(); }}><Icon name="folder"/><span>{item.environment.name}<small>{item.configPath}</small></span>{environment?.configPath === item.configPath && environment.revision === item.revision && <Icon name="check"/>}</button> : <button key={item.configPath} type="button" onClick={() => onOpenEnvironmentSettings?.()}><Icon name="refresh"/><span>Unreadable environment<small>{item.error}</small></span></button>)}<button role="menuitem" type="button" onClick={() => { close(); onOpenEnvironmentSettings?.(); }}><Icon name="plus"/><span>Set up project</span></button></div>}</>}
