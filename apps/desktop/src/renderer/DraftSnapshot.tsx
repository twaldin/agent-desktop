...
export function DraftSnapshot({ draft, hostName, projects, media, hostId, connected = false }: { draft: Draft; hostName: string; projects: Project[]; media?: AttachmentMediaContext; hostId?: string; connected?: boolean }) {
  ...
  return <div className="draft-snapshot">
    ...
    </dl><pre>{draft.text || ((draft.attachments?.length || draft.selectedTextAttachments?.length || draft.wholeFileAttachments?.length) ? "(No authored text)" : "(Empty draft)")}</pre>
    {draft.wholeFileAttachments?.length ? <div className="draft-snapshot-attachments"><strong>Files</strong><ol>{draft.wholeFileAttachments.map(item => <li key={item.id}>{item.source.path} · {item.source.hostId}</li>)}</ol></div> : null}
