...
export function OrganizedSidebar(props: Props) {
  ...
  function toggleMenu(id: string, button: HTMLButtonElement) { clearHover(); menuTrigger.current = button; const bounds = button.getBoundingClientRect(); setMenuPosition({left: Math.min(bounds.right + 4, window.innerWidth - 216), top: Math.max(8, Math.min(bounds.top, window.innerHeight - 248))}); setMenu(value => value === id ? undefined : id); }
