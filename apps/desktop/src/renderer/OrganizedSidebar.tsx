...
export function OrganizedSidebar(props: Props) {
  ...
  function organizationMenu(kind: "project" | "chat") {
    ...
    return <div className="menu-anchor"><button className="icon-button small" aria-label={kind === "project" ? "Project sidebar options" : "Chat sidebar options"} aria-expanded={menu === id} onClick={event => toggleMenu(id, event.currentTarget)}><Icon name="more"/></button>{menu === id && createPortal(<><button className="menu-dismiss" aria-label="Close sidebar options" tabIndex={-1} onClick={() => setMenu(undefined)}/><div ref={menuRef} className="action-menu sidebar-organization-menu" style={menuPosition} onKeyDown={menuKeys} role="menu" aria-label="Sidebar organization and sorting">
      ...
      {kind === "chat" && <button onClick={() => { setMenu(undefined); props.onToggleArchived(); }}>{showArchived ? "Show active chats" : "Show archived chats"}</button>}
      <button disabled={!writable} onClick={() => { setMenu(undefined); setDialog({ name: "" }); }}>New section</button>
    </div></>, document.body)}</div>;
  }
  function pinnedMenu() {
    const id = "options:pinned", sort = data.pinnedSort?.() ?? "manual";
    return <div className="menu-anchor"><button className="icon-button small" aria-label="Pinned options" aria-expanded={menu === id} onClick={event => toggleMenu(id, event.currentTarget)}><Icon name="more"/></button>{menu === id && createPortal(<><button className="menu-dismiss" aria-label="Close pinned options" tabIndex={-1} onClick={() => setMenu(undefined)}/><div ref={menuRef} className="action-menu sidebar-organization-menu" style={menuPosition} onKeyDown={menuKeys} role="menu" aria-label="Pinned options">
      {([["priority","Priority"],["updated_at","Last updated"],["manual","Manual order"]] as const).map(([value,label]) => <button key={value} role="menuitemradio" aria-checked={sort === value} disabled={!writable} onClick={() => { setMenu(undefined); void data.put({ key: "sidebar.pinnedSort", value }); }}>{label}<span aria-hidden="true">{sort === value ? "✓" : ""}</span></button>)}
    </div></>, document.body)}</div>;
  }
  ...
  return <div className="organized-sidebar">
    ...
    {pinned.length > 0 && <section aria-label="Pinned">{sectionHeading("pinned", "Pinned", pinnedMenu())}{!isCollapsed("pinned") && pinned.map(item => render(item, pinned))}</section>}
    ...
    <div className="conversation-heading">{sectionHeading("recents", showArchived ? "Archived chats" : "Recents", <div className="sidebar-section-actions">{organizationMenu("chat")}<button className="icon-button small" onClick={() => props.onNew(undefined, props.activeHostId)} title="New chat" aria-label="New chat"><Icon name="compose"/></button></div>)}</div>
