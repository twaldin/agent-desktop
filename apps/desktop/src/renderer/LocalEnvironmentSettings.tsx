    ...
    <div className="local-environment-layout">
      <div className="local-environment-editor">
        ...
        {editor && editing && config && !rawEditor && <form onSubmit={submit} className="local-environment-form">
          ...
          <section className="local-environment-actions"><div className="local-environment-section-heading"><h3>Actions</h3><button type="button" className="secondary-button" onClick={addAction}>Add action</button></div><p className="local-environment-actions-description">These actions can run any command and will be displayed in the header.</p><div className="local-environment-action-list">{(config.actions ?? []).map((action, index) => <ActionEditor key={index} action={action} onChange={change => editAction(index, change)} onRemove={() => removeAction(index)} />)}{!config.actions?.length && <p className="local-environment-empty">Add a named command to this environment.</p>}</div></section>
...
function ActionEditor({ action, onChange, onRemove }: { action: LocalEnvironmentAction; onChange(change: Partial<LocalEnvironmentAction>): void; onRemove(): void }) {
  const nameId = useId(), commandId = useId();
  return <div className="local-environment-action"><div className="local-environment-action-fields">
    <div className="local-environment-action-field"><label htmlFor={nameId}>Name</label><div className="local-environment-action-name"><EnvironmentActionIconPicker value={action.icon} onChange={icon => onChange({ icon })}/><input id={nameId} value={action.name} onChange={event => onChange({ name: event.target.value })} /></div></div>
    <div className="local-environment-action-field"><label htmlFor={commandId}>Action script</label><textarea id={commandId} rows={4} value={action.command} onChange={event => onChange({ command: event.target.value })} spellCheck={false} /></div>
    <div className="local-environment-action-footer"><div className="local-environment-action-field"><span className="local-environment-action-label">Platforms</span><div className="local-environment-action-platforms" role="group" aria-label="Action platform">
      {([undefined, ...platforms.slice(1)] as Array<LocalEnvironmentPlatform | undefined>).map(value => { const selected = action.platform === value; return <button key={value ?? "all"} type="button" role="checkbox" aria-checked={selected} className={selected ? "selected" : ""} onClick={() => onChange({ platform: value })}>{value ? platformLabel(value) : "All platforms"}</button>; })}
    </div></div><button type="button" className="icon-button small local-environment-action-delete" aria-label="Delete" title={`Delete ${action.name || "action"}`} onClick={onRemove}><Icon name="close" /></button></div>
  </div></div>;
}
