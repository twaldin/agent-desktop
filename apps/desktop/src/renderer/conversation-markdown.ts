...
import { goalCompletionTitle, messageBlocks, toolLinks, toolOutcome, type ToolLink } from "./transcript-state";
...
export function conversationMarkdownIssue(snapshot: ConversationMarkdownSnapshot, owner: ConversationMarkdownOwner, copyPending = false): string | null {
  if (copyPending) return "Wait for the current copy to finish before copying again.";
...
export function captureConversationMarkdown(snapshot: ConversationMarkdownSnapshot, owner: ConversationMarkdownOwner, title: string): string {
  ...
  const links = toolLinks([...snapshot.messages]);
  const sections = snapshot.messages.map(message => messageMarkdown(message, links.results.get(message.id))).filter(section => section.trim().length > 0);
...
function blockMarkdown(block: TranscriptBlock, output: boolean): string {
  switch (block.type) {
    ...
    default: return "[A native block contains unsupported content; its payload is not included.]";
  }
}
...
function messageMarkdown(message: TranscriptMessage, linkedCall?: ToolLink): string {
  ...
  if (message.role === "fileMention" && message.fileReferences) return message.fileReferences.map(file => {
    const qualification = file.image
      ? `[Recorded image (${label(file.image.mimeType)}); image data is not included.${file.image.error ? ` ${label(file.image.error)}` : ""}]`
      : file.skippedReason ? `[File content was not recorded: ${file.skippedReason === "binary" ? "binary file" : "file too large"}.]` : "";
    const contents = [file.content || !qualification ? fenced(file.content) : "", qualification].filter(Boolean).join("\n\n");
    return `File context: ${inline(file.path)}\n\n${contents}`;
  }).join("\n\n");
  if (message.role === "selectedText" && message.selectedText) return ["Saved selected context · prompt not linked", ...selectedContext(message)].join("\n\n");
  ...
  if (output) {
    sections.push(`Tool result: ${inline(message.tool?.name ?? linkedCall?.call.name ?? "Tool")} — ${toolOutcome(message).label}`);
  ...
  if (message.role === "assistant" && message.goalCompletion) {
    const completion = message.goalCompletion;
    sections.push(`Goal achieved\n\n${quoted(completion.objective)}\n\n${goalCompletionTitle(completion)}`);
  }
