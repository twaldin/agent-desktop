...
const clone = (state: DockState): DockState => ({
  ...state,
...
export function validateDockState(
  ...
  const region = (candidate: unknown): DockRegion => {
    const source =
      candidate && typeof candidate === "object"
        ? (candidate as Partial<DockRegion>)
        : {};
    const seen = new Set<string>();
    const tabIds = Array.isArray(source.tabIds)
      ? source.tabIds.filter(
          (id): id is string =>
            typeof id === "string" &&
            known.has(id) &&
            !seen.has(id) &&
    ...
    const activeTabId =
      ...
      tabIds.includes(source.activeTabId)
        ? source.activeTabId
        : tabIds[0];
    return { tabIds, activeTabId, open: source.open === true };
  };
  const valid = new Set(
    tabs.filter((tab) => tab.id === dockTabId(tab)).map((tab) => tab.id),
  );
  const right = region({
      ...(raw.right as object),
      tabIds: (raw.right as DockRegion | undefined)?.tabIds?.filter((id) =>
        valid.has(id),
      ),
    }),
    bottom = region({
      ...(raw.bottom as object),
      tabIds: (raw.bottom as DockRegion | undefined)?.tabIds?.filter((id) =>
        valid.has(id),
      ),
    });
  for (const id of right.tabIds) {
    const at = bottom.tabIds.indexOf(id);
    if (at >= 0) bottom.tabIds.splice(at, 1);
  }
  ...
  const state: DockState = {
    ...
    rightWidthRatio:
      typeof raw.rightWidthRatio === "number"
        ? raw.rightWidthRatio
        : fallback.rightWidthRatio,
    bottomHeight:
      typeof raw.bottomHeight === "number"
        ? raw.bottomHeight
  ...
  return resizeDock(
    resizeDock(
      state,
      "right",
      state.rightWidthRatio * viewport.width,
      viewport,
    ),
    "bottom",
    state.bottomHeight,
...
export function insertDockTab(
  ...
  const currentDestination = state.right.tabIds.includes(tab.id)
    ? "right"
    : state.bottom.tabIds.includes(tab.id)
      ? "bottom"
      : undefined;
  if (currentDestination && !move)
    return activateDockTab(state, currentDestination, tab.id);
  const next = clone(state);
  for (const region of [next.right, next.bottom]) {
    const current = region.tabIds.indexOf(tab.id);
    if (current >= 0) region.tabIds.splice(current, 1);
    if (region.activeTabId === tab.id)
      region.activeTabId = region.tabIds[current] ?? region.tabIds[current - 1];
  }
...
export function closeDockTab(
  ...
  const next = clone(state),
    region = next[destination],
    index = region.tabIds.indexOf(id);
  if (index < 0) return state;
  region.tabIds.splice(index, 1);
  if (region.activeTabId === id)
    region.activeTabId = region.tabIds[index] ?? region.tabIds[index - 1];
...
export function moveDockTab(
  ...
  const from = state.right.tabIds.includes(id)
    ? "right"
    : state.bottom.tabIds.includes(id)
      ? "bottom"
      : undefined;
...
export function reorderDockTab(
  ...
  const region = state[destination],
    current = region.tabIds.indexOf(id);
  if (current < 0) return state;
  const next = clone(state),
    ids = next[destination].tabIds;
  ids.splice(current, 1);
  ids.splice(clamp(index, 0, ids.length), 0, id);
  return next;
}
export function resizeDock(
  ...
  if (destination === "bottom") {
    const max = height / 2,
      min = Math.min(160, max),
      fallback = state.bottomHeight;
    next.bottomHeight = clamp(
      Number.isFinite(size) ? size : fallback,
      min,
      max,
    );
  } else {
    const requestedMin = (viewport.rightMinWidth ?? 320) / Math.max(width, 1),
      requestedMax = 1 - 352 / Math.max(width, 1),
      max = Math.max(0, requestedMax),
      min = Math.min(Math.max(0, requestedMin), max),
      fallback = state.rightWidthRatio * width;
    next.rightWidthRatio = clamp(
      (Number.isFinite(size) ? size : fallback) / Math.max(width, 1),
      min,
      max,
    );
  }
