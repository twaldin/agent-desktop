  ...
  return <section className="settings-page general-settings" aria-label="General settings">
    <div className="general-settings-scroll">
      ...
      <section className="general-settings-group" aria-labelledby="general-interaction"><h2 id="general-interaction">Interaction</h2><div className="general-settings-card">
        <div className="general-settings-row"><div><h3>Send messages with</h3><p>Choose when Enter sends a prompt or inserts a new line</p></div><select aria-label="Send messages with" disabled={!writable} value={preferences.get('general.sendBehavior') ?? 'enter'} onChange={event => void preferences.put({key:'general.sendBehavior',value:event.target.value as 'enter'|'mod-enter-if-multiline'|'mod-enter'})}><option value="enter">Enter</option><option value="mod-enter-if-multiline">⌘ + Enter for multiline prompts</option><option value="mod-enter">⌘ + Enter always</option></select></div>
        <div className="general-settings-row"><div><h3 id="general-follow-up-behavior">Follow-up behavior</h3><p>Queue follow-ups while Agent Desktop runs or steer the current run. Use the modified send shortcut to do the opposite for one message.</p></div><div className="general-segmented" role="group" aria-labelledby="general-follow-up-behavior">{(['queue','steer'] as const).map(mode => <button key={mode} type="button" aria-pressed={(preferences.get('general.followUpQueueMode') ?? 'steer') === mode} disabled={!writable} onClick={() => void preferences.put({key:'general.followUpQueueMode',value:mode})}>{mode === 'queue' ? 'Queue' : 'Steer'}</button>)}</div></div>
      </div></section>
