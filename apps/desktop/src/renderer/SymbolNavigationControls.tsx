...
export function SymbolNavigationControls({ binding, frame, active, capture }: {
  ...
  const { navigation, path, open } = binding;
  ...
  useEffect(() => {
    ...
    const command = (event: Event) => {
      ...
      const { navigation, path, open } = value.binding;
      if (event.detail === "file.goToDefinition" && !navigation.unavailable(path)) { event.preventDefault(); void navigation.define(path, value.capture(), open); }
      else if (event.detail === "file.navigateBack" && navigation.canBack && navigation.data.connected) { event.preventDefault(); void navigation.travel(-1, path, value.capture(), open); }
      else if (event.detail === "file.navigateForward" && navigation.canForward && navigation.data.connected) { event.preventDefault(); void navigation.travel(1, path, value.capture(), open); }
    };
  ...
  return <div className="symbol-navigation" data-symbol-navigation>
    <div className="symbol-navigation-actions" role="group" aria-label="Symbol navigation">
      <button type="button" data-symbol-command="file.goToDefinition" disabled={navigation.busy || Boolean(unavailable)} title={unavailable ?? "Resolve the identifier at the editor cursor with TypeScript 7.0.2"}
        onClick={() => void navigation.define(path, capture(), open)}>Go to definition</button>
      <button type="button" data-symbol-command="file.navigateBack" disabled={!navigation.canBack || !navigation.data.connected} onClick={() => void navigation.travel(-1, path, capture(), open)}>Back to symbol</button>
      <button type="button" data-symbol-command="file.navigateForward" disabled={!navigation.canForward || !navigation.data.connected} onClick={() => void navigation.travel(1, path, capture(), open)}>Forward to symbol</button>
    ...
    {navigation.choice && navigation.choice.definitions.length > 1 && <div className="symbol-definition-choices" role="group" aria-label="Choose a symbol definition" onKeyDown={event => { if (event.key === "Escape") { event.preventDefault(); navigation.cancel(); } }}>
      {navigation.choice.definitions.map((definition, index) => <button type="button" key={`${definition.path}:${definition.selection.start.line}:${definition.selection.start.column}:${index}`} disabled={navigation.busy}
        onClick={() => void navigation.choose(index, open)} aria-label={`Open ${definition.path}, line ${definition.selection.start.line}, column ${definition.selection.start.column}`}>
