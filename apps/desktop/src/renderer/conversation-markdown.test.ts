...
import { captureConversationMarkdown, conversationMarkdownIssue, type ConversationMarkdownSnapshot } from "./conversation-markdown";
...
test("keeps captured selected text and native file qualifications without rereading paths or exporting image bytes", () => {
  const copied = captureConversationMarkdown(snapshot([
    ...
    { id: "f", role: "fileMention", text: "", fileReferences: [{ path: "/workspace/large.dat", content: "", skippedReason: "tooLarge" }, { path: "/workspace/image.png", content: "Recorded caption", image: { blockIndex: 1, mimeType: "image/png" } }] },
...
test("uses the displayed tool association without inventing a name for ambiguous reused calls", () => {
  const call = (id: string, name: string): TranscriptMessage => ({ id, role: "assistant", text: "", content: [{ type: "toolCall", id: "call", name, arguments: {} }] });
  const result: TranscriptMessage = { id: "result", role: "toolResult", text: "recorded output", tool: { callId: "call", status: "completed", isError: false } };
  expect(captureConversationMarkdown(snapshot([call("first", "read"), result]), owner, "Title")).toContain("Tool result: ` read ` — Completed");
  expect(captureConversationMarkdown(snapshot([call("first", "read"), call("second", "write"), result]), owner, "Title")).toContain("Tool result: ` Tool ` — Completed");
});

test("retains recorded goal completion while qualifying unknown blocks and missing selected context", () => {
  const future = JSON.parse('{"type":"futureBlock","payload":"PRIVATE-FUTURE"}');
  const copied = captureConversationMarkdown(snapshot([
    { id: "a", role: "assistant", text: "Done", content: [{ type: "text", text: "Done" }, future], goalCompletion: { entryId: "goal", objective: "Keep **literal** objective", tokensUsed: 23, tokenBudget: 50, timeUsedSeconds: 9 } },
    { id: "missing", role: "selectedText", text: "PRIVATE-UNBOUND" },
  ]), owner, "Title");
  expect(copied).toContain("Goal achieved");
  expect(copied).toContain("Keep **literal** objective");
  expect(copied).toContain("23 tokens used");
  expect(copied).toContain("50 token budget");
  expect(copied).toContain("9s active time");
  expect(copied).toContain("unsupported content");
  expect(copied).toContain("Native selectedText message is not supported");
  expect(copied).not.toContain("Saved selected context");
  expect(copied).not.toContain("PRIVATE-");
});

test("explains a pending write even when the next conversation has a ready transcript", () => {
  const ready = snapshot([{ id: "a", role: "assistant", text: "Ready" }]);
  expect(conversationMarkdownIssue(ready, owner, true)).toBe("Wait for the current copy to finish before copying again.");
  expect(conversationMarkdownIssue(ready, owner, false)).toBeNull();
});
