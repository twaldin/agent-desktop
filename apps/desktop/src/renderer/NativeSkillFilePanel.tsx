...
export function NativeSkillFilePanel({ controller, connected, active, fileMode, onFileModeChange, fileScroll, onFileScrollChange, onAddToChat, openExternal }: {controller:NativeSkillFileController;connected:boolean;active:boolean;fileMode?:"markdown"|"source";onFileModeChange?(mode:"markdown"|"source"):void;fileScroll?:{markdown?:number;source?:number};onFileScrollChange?(mode:"markdown"|"source",top:number):void;onAddToChat?(path:string,selection:FileTextSelection):void;openExternal?(url:string):Promise<void>}) {
  ...
  return <section className="native-skill-file-panel" hidden={!active} tabIndex={-1} aria-label="Skill file editor">
    ...
    {data.loading&&!data.file?<p role="status">Loading skill file…</p>:data.file&&<div className="native-skill-file-body">
      ...
      <RichMarkdownEditor documentKey={documentKey} value={data.text} label="Skill file Markdown" active={active&&!source}
        onAddToChat={onAddToChat?selection=>onAddToChat(data.ref.sourcePath,selection):undefined} initialScrollTop={fileScroll?.markdown} onScrollChange={top=>onFileScrollChange?.("markdown",top)}
      ...
      <PierreSourceEditor documentKey={documentKey} name="SKILL.md" value={data.text} label="Skill file source" active={active&&source}
        onAddToChat={onAddToChat?selection=>onAddToChat(data.ref.sourcePath,selection):undefined} initialScrollTop={fileScroll?.source} onScrollChange={top=>onFileScrollChange?.("source",top)}
