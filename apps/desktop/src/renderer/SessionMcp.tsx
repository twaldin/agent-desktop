...
export function SessionMcp({ bridge, hostId, sessionId, connected, idle = false, mutationPending = false, query = "", target, onBack }: { bridge: DesktopBridge; hostId: string; sessionId: string; connected: boolean; idle?: boolean; mutationPending?: boolean; query?: string; target?: WorkspaceTarget; onBack?(): void }) {
  ...
  useEffect(() => {
    ...
    setPending(pendingId); setSnapshot(null); setUnavailable(null); setCommandError(null); setReloading(false); writing.current=false;
    const read = async () => {
      ...
      try {
        ...
        if (value.receipt) {
          if (value.receipt.state==='succeeded' || value.receipt.state==='failed') {
            try { localStorage.removeItem(storageKey); } catch { setCommandError('Could not clear the saved reload receipt.'); return; }
            pendingId=null; setPending(null);
            setCommandError(value.receipt.state==='failed' ? value.receipt.message ?? 'Native reload failed.' : null);
          } else setCommandError(value.receipt.state==='pending' ? 'Waiting for the original reload to finish.' : 'The previous reload outcome is unconfirmed. It has not been replayed. Inspect the owning host before starting another reload.');
        }
      } catch(error) {
  ...
  const reload = async () => {
    ...
    try { localStorage.setItem(storageKey,id); } catch {setCommandError('Cannot preserve a reload receipt on this device. No reload was sent.');return;}
    writing.current=true;readRevision.current++;setReloading(true);setPending(id);setCommandError(null);
    try {
      const result=await bridge.command({id,command:{type:'session.mcp.reload',sessionId,epoch:snapshot.epoch,expectedRevision:snapshot.revision}},hostId);
      if(current!==generation.current)return;
      if(result.commandId!==id)throw new Error('The host returned another command receipt.');
      if(!result.ok)throw new Error(result.error.message);
      if(!result.value || !('type' in result.value) || result.value.type!=='session.mcp')throw new Error('The host returned no native MCP reload snapshot.');
      setSnapshot(result.value.snapshot);
    } catch(error) {if(current===generation.current)setCommandError(errorMessage(error));}
    finally {
      if(current===generation.current){writing.current=false;setReloading(false);setRefresh(value=>value+1);}
    }
  };
  return <section className="session-mcp" aria-label="Live MCP servers">
    ...
    {unavailable && <p className="inline-error" role="alert">{unavailable} Refresh session state before trying a native reload.</p>}
    ...
    {snapshot?.available && snapshot.servers.filter(server=>`${server.name} ${server.tools.join(" ")}`.toLowerCase().includes(query.toLowerCase())).map(server => <article className="session-mcp-row" key={server.name}><div><strong>{server.name}</strong><small>{server.source} · {server.status}</small>{server.error && <p className="inline-error">{server.error}</p>}{server.tools.length>0 && <details><summary><Icon name="chevron"/>Tools</summary><ul>{server.tools.map(name=><li key={name}>{name}</li>)}</ul></details>}</div><div className="session-mcp-counts"><span>{server.tools.length} {server.tools.length===1?"tool":"tools"}</span><span>{server.resourceCount === null ? "Resources unmeasured" : `${server.resourceCount} ${server.resourceCount===1?"resource":"resources"}`}</span><span>{server.promptCount === null ? "Prompts unmeasured" : `${server.promptCount} ${server.promptCount===1?"prompt":"prompts"}`}</span></div></article>)}
  </section>;
}
