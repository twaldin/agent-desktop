...
export function App() {
  ...
  useEffect(() => {
    ...
    const apply = () => {
      ...
      try { applyTheme(appliedTheme); backgroundColor = cssColorToRgba(getComputedStyle(document.documentElement).getPropertyValue("--app-surface").trim()); }
      catch (cause) { setThemeEffectsError(errorMessage(cause)); return; }
      void bridge.applyWindowTheme({ material: appliedTheme.material, opaqueWindows: appliedTheme.opaqueWindows, backgroundColor }).then(() => { if (!cancelled) setThemeEffectsError(undefined); }, cause => { if (!cancelled) setThemeEffectsError(errorMessage(cause)); });
    };
  ...
  const organizedSidebar = sidebarLayout(preferences, hostGroups, "", showArchived, expandedProjects);
