...
import { SessionProcessesPanel } from "./SessionProcessesPanel";
import { createSessionProcessesJournal } from "./session-process-journal";
...
export function App() {
  ...
  const processesJournal = useMemo(() => createSessionProcessesJournal(window.agentDesktopWindow), []);
  ...
  function renderDockTab(tab:DockTab, active = true) {
    ...
    if (tab.kind === "subagents") {
      if (!("sessionId" in target)) return <p>Subagents belong to an original conversation.</p>;
      return <SubagentsPanel key={owner} bridge={bridge} hostId={tab.hostId} sessionId={target.sessionId} connected={online} active={active} media={attachmentMedia}/>;
    }
