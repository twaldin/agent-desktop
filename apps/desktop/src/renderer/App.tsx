...
export function App() {
  ...
  const dockActions = workspaceTarget ? [
    ...
    ...("sessionId" in workspaceTarget ? [{id:"browser",label:"Browser",onSelect:(destination:DockDestination) => { void dock.browser(destination, true); }}, {id:"existing-browser",label:"Existing browser tabs",onSelect:(destination:DockDestination) => { void dock.browser(destination, false); }}] : []),
  ] : [];
  ...
  return <div className={`app-shell ${sidebarOpen ? "" : "sidebar-hidden"}`}>
      ...
      {loading && !state ? <div className="center-state"><span className="spinner"/><h1>Connecting to your host</h1><p>Loading projects, models, and saved conversations.</p></div> : missingSession ? <div className="center-state"><h1>Conversation unavailable</h1><p>{connected ? "This conversation is not in the owning host’s catalog." : "This conversation is not in this device’s cached catalog. Its owning host is unavailable; the selected conversation is preserved."}</p><button className="secondary-button" onClick={() => navigate(null)}>New conversation</button></div> : <>
        ...
        <div className={`composer-region ${selectedId ? "" : "home-composer"}`}>
          ...
          {!selectedId && <ComposerContext ref={composerContext} hostId={hostId} hostName={state?.host.name ?? hostId} hosts={desktop.hosts} projects={state?.projects ?? []} projectId={draft.projectId} connected={connected} addingProject={addingProject} workspace={workspace}
            branchPrefix={preferences.get("git.branchPrefix") ?? "codex/"} onBranchPrefixChange={async prefix => { if (preferences.pending.length || preferences.busy) throw new Error("A shared preference change is already in progress."); await preferences.put({ key: "git.branchPrefix", value: prefix }); if (preferences.error) throw new Error(preferences.error); }}
    ...
    {(["right", "bottom"] as const).map(destination => <div className={`dock-slot dock-slot-${destination}`} key={destination} style={{display:!settingsOpen && dock.snapshot.state[destination].open ? undefined : "none"}} inert={settingsOpen || !dock.snapshot.state[destination].open || undefined}>
      ...
      {!dock.snapshot.state[destination].tabIds.length && <div className="dock-empty-actions">{dockActions.map(action => <button key={action.id} onClick={() => action.onSelect(destination)}><Icon name={(action.id === "browser" || action.id === "existing-browser") ? "globe" : action.id === "terminal" ? "terminal" : "folder"}/>{action.label}</button>)}</div>}
    </div>)}
    </div>
