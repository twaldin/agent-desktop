...
function catalogue(dock:unknown,overrides:Record<string,unknown>={}) {return build({dockWorkspace:{sessionId:"session"},dockSession:{sessionId:"session"},hostId:"owner",dock,connected:true,bridge:{getBtw:()=>{}},workspace:{status:{}},commandKeymap:undefined,appCommandBindings:{bindings:{}},workspaceKey,dockTabId,appCommandShortcutLabel:()=>"shortcut",hasNativeTerminalBridge:()=>true,browserMenu:new BrowserWorkspaceMenu(()=>{}),dockEmptyActionCatalogue,state:undefined,draftDockOwner:{enabled:false},committedDraftDockOwner:{current:undefined},draftId:"draft",draftBrowserDockTarget,createDraftBrowserDockTab,mcpActions:[],...overrides});}
