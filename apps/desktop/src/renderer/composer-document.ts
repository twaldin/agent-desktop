...
import { parseWholeFileAttachments, parseInlineWholeFileMentions, hasRepeatedWholeFileSources, type WholeFileAttachment } from '@agent-desktop/shared';
...
export const composerSchema = new Schema({nodes:{
 ...
 file:{inline:true,group:'inline',atom:true,selectable:false,draggable:false,attrs:{id:{},hostId:{},path:{}},toDOM:node=>['span',{'data-file-id':node.attrs.id,'data-agent-desktop-file':'1','data-agent-desktop-host':node.attrs.hostId,'at-mention-label':node.attrs.path.split('/').at(-1),'at-mention-path':node.attrs.path,'at-mention-fs-path':node.attrs.path,contenteditable:'false',class:'composer-inline-file'},node.attrs.path.split('/').at(-1)]},
}});
...
export function composerDocument(text:string,files:readonly WholeFileAttachment[]=[]):ProseMirrorNode {
 const checked=(hasRepeatedWholeFileSources(files)?parseInlineWholeFileMentions(files,text.length):parseWholeFileAttachments(files,text.length));
...
export function readComposerDocument(doc:ProseMirrorNode):ComposerDocument {
 ...
 return {text,files:(hasRepeatedWholeFileSources(files)?parseInlineWholeFileMentions(files,text.length):parseWholeFileAttachments(files,text.length))};
}
