...
test("actual App selection routes to the original host draft and refuses a replaced committed page", () => {
  ...
  try {
    ...
    const select = () => invoke(entry, dock, committed, (message: string) => errors.push(message), route, false, false, navigate, () => { frames++; }, undefined, undefined, activateBrowserSearchTab, selection);
...
test.each(["removed", "replaced", "retained"] as const)("actual App queued draft page admission controls route and frame (%s)", kind => {
  ...
  try {
    ...
    const select = () => invoke(entry, dock, committed, (message: string) => errors.push(message), route, false, false,
