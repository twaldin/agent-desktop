...
export function NativeIntegrations({ bridge, hostId, hostName, connected, target, page, onClose, onPageChange, sessionIdle = false }: NativeIntegrationsProps) {
  ...
  useEffect(() => {
    ...
    setSaving(false); setSelected(null); setLiveMcp(false); setPlugins(null); setMcp(null); setQuery("");
  ...
  return <section className="settings-page native-integrations" aria-label={page === "plugins" ? "Plugins" : "MCP servers"}>
    ...
    {page === "plugins" ? <PluginPage catalog={plugins ? {...plugins, plugins:plugins.plugins.filter(p => `${p.name} ${p.title} ${p.description ?? ""}`.toLowerCase().includes(query.toLowerCase()))} : null} selected={plugin} connected={connected} saving={saving} setSaving={setSaving} setCatalog={setPlugins} setError={setError} bridge={bridge} target={target} hostId={hostId} onSelect={setSelected}/> : liveMcp && target && "sessionId" in target ? <SessionMcp bridge={bridge} hostId={hostId} sessionId={target.sessionId} target={target} connected={connected} idle={sessionIdle} mutationPending={saving} query={query} onBack={() => setLiveMcp(false)}/> : <McpPage key={`${hostId}:${targetKey}`} catalog={mcp ? {...mcp, servers:mcp.servers.filter(s => s.name.toLowerCase().includes(query.toLowerCase()))} : null} connected={connected} saving={saving} setSaving={setSaving} setCatalog={setMcp} setError={setError} bridge={bridge} target={target} hostId={hostId} onLive={() => setLiveMcp(true)}/>}
    </div>
  </section>;
}
