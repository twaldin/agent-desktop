...
export function NativeIntegrations({ bridge, hostId, hostName, connected, target, page, onClose, onPageChange }: NativeIntegrationsProps) {
  ...
  useEffect(() => { setSelected(null); setPlugins(null); setMcp(null); setQuery(""); void reload(); return () => { epoch.current++; }; }, [page, hostId, targetKey, connected]);
  ...
  return <section className="settings-page native-integrations" aria-label={page === "plugins" ? "Plugins" : "MCP servers"}>
    ...
    <header className="integration-heading"><div><h1>Plugins</h1><p>Manage plugins and MCPs on {hostName}</p></div><button className="icon-button" aria-label="Reload integrations" title="Reload integrations" disabled={!connected || loading || saving} onClick={() => void reload()}><Icon name="refresh"/></button></header>
    <div className="integration-navigation"><div role="tablist" aria-label="Integrations"><button role="tab" aria-selected={page === "plugins"} onClick={() => onPageChange?.("plugins")}>Plugins{plugins ? ` ${plugins.plugins.length}` : ""}</button><button role="tab" aria-selected={page === "mcp"} onClick={() => onPageChange?.("mcp")}>MCPs{mcp ? ` ${mcp.servers.length}` : ""}</button></div><label className="integration-search"><Icon name="search"/><input type="search" aria-label={page === "plugins" ? "Search plugins" : "Search MCP servers"} placeholder={page === "plugins" ? "Search plugins" : "Search MCP servers"} value={query} onChange={event => setQuery(event.target.value)}/></label></div>
    ...
    {page === "plugins" ? <PluginPage catalog={plugins ? {...plugins, plugins:plugins.plugins.filter(p => `${p.name} ${p.title} ${p.description ?? ""}`.toLowerCase().includes(query.toLowerCase()))} : null} selected={plugin} connected={connected} saving={saving} setSaving={setSaving} setCatalog={setPlugins} setError={setError} bridge={bridge} target={target} hostId={hostId} onSelect={setSelected}/> : <McpPage catalog={mcp ? {...mcp, servers:mcp.servers.filter(s => s.name.toLowerCase().includes(query.toLowerCase()))} : null} connected={connected} saving={saving} setSaving={setSaving} setCatalog={setMcp} setError={setError} bridge={bridge} target={target} hostId={hostId}/>}
    </div>
  </section>;
}
...
function McpPage({ catalog, connected, saving, setSaving, setCatalog, setError, bridge, target, hostId }: { catalog: NativeMcpCatalog | null; connected: boolean; saving: boolean; setSaving: (v: boolean) => void; setCatalog: (v: NativeMcpCatalog) => void; setError: (v: string | null) => void; bridge: DesktopBridge; target?: WorkspaceTarget; hostId: string }) {
  const [showAdd, setShowAdd] = useState(false); const [advanced, setAdvanced] = useState(""); const [name, setName] = useState(""); const [transport, setTransport] = useState<"stdio" | "http" | "sse">("stdio"); const [source, setSource] = useState(""); const [args, setArgs] = useState(""); const [scope, setScope] = useState<"user" | "project">("user"); const [adding, setAdding] = useState(false); const epochForMutations = useRef(0);
  const targetKey = JSON.stringify(target);
  useEffect(() => { if (!target) setScope("user"); return () => { epochForMutations.current++; }; }, [hostId, targetKey, connected]);
  ...
  const add = async (event: FormEvent) => { event.preventDefault(); if (!name.trim() || !source.trim() || !catalog) return; let extra: Record<string, unknown> = {}; try { if (advanced.trim()) { const parsed: unknown = JSON.parse(advanced); if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) throw new Error(); extra = parsed as Record<string, unknown>; } } catch { setError("Additional settings must be a JSON object."); return; } setAdding(true); const success = await mutate({ expectedRevision: catalog.revision, operation: "add", scope, name: name.trim(), config: transport === "stdio" ? { ...extra, type: "stdio", command: source.trim(), args: args.split("\n").filter(Boolean) } : { ...extra, type: transport, url: source.trim() } }); if (success) { setName(""); setSource(""); setArgs(""); setAdvanced(""); setShowAdd(false); } setAdding(false); };
  return <div className="mcp-layout">{!showAdd ? <section className="mcp-catalog"><div className="integration-list-heading"><h2>MCP servers</h2><button className="primary-button" disabled={!connected || saving || !catalog} onClick={() => setShowAdd(true)}>Add MCP</button></div>{!catalog && <p role="status">{connected ? "Loading MCP servers…" : "MCP catalog unavailable offline."}</p>}{catalog?.servers.map(server => <article className="mcp-row" key={server.id}><div><strong>{server.name}</strong><small>{mcpTransportLabel(server.transport)}{server.shadowed ? " · Shadowed" : ""} · {server.scope} · {server.source}</small></div><label><input type="checkbox" checked={server.enabled} disabled={!connected || saving} onChange={event => void mutate({ expectedRevision: catalog.revision, operation: "enabled", serverId: server.id, enabled: event.target.checked })}/> Enabled</label>{server.removable && <button className="secondary-button" disabled={!connected || saving} onClick={() => { if (window.confirm(`Remove MCP server “${server.name}”?`)) void mutate({ expectedRevision: catalog.revision, operation: "remove", serverId: server.id }); }}>Remove</button>}</article>)}</section> : <><button className="integration-back" type="button" onClick={() => setShowAdd(false)}><Icon name="browserBack"/> Back</button><form className="mcp-add" onSubmit={add}><h2>Connect to a custom MCP</h2><label>Name<input className="text-field" value={name} onChange={event => setName(event.target.value)} disabled={!connected || adding}/></label><label>Scope<select value={scope} onChange={event => setScope(event.target.value as "user" | "project")} disabled={!connected || adding || !target}><option value="user">User</option><option value="project">Project</option></select></label><div className="integration-type-row integration-form-row"><span>Type</span><div className="integration-segments" role="group" aria-label="MCP server type">{(["stdio", "http", "sse"] as const).map(type => <button key={type} type="button" aria-pressed={transport === type} disabled={!connected || adding} onClick={() => setTransport(type)}>{type === "stdio" ? "STDIO" : type === "http" ? "Streamable HTTP" : "SSE"}</button>)}</div></div><label>{transport === "stdio" ? "Command" : "URL"}<input aria-label={transport === "stdio" ? "Command to launch" : "Server URL"} className="text-field" value={source} onChange={event => setSource(event.target.value)} disabled={!connected || adding}/></label>{transport === "stdio" && <label>Arguments<textarea className="text-field" value={args} onChange={event => setArgs(event.target.value)} disabled={!connected || adding} placeholder="One argument per line"/></label>}<details className="mcp-advanced"><summary>Additional settings</summary><label><span>Native server configuration</span><textarea aria-label="Additional MCP settings" className="text-field" value={advanced} onChange={event => setAdvanced(event.target.value)} disabled={!connected || adding} placeholder={'{"env": {}, "cwd": ""}'}/></label><p className="integration-note">Environment variables, headers, working directory, timeouts and authentication options.</p></details><button className="primary-button" type="submit" disabled={!connected || adding || !name.trim() || !source.trim() || !catalog}>Save</button><p className="integration-note">Configuration is parsed and applied by the owning native host for new sessions.</p></form></>}</div>;
}
