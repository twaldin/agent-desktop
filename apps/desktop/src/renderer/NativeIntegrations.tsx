...
export function NativeIntegrations({ bridge, hostId, hostName, connected, target, page, onClose, onPageChange, onBrowse, onTrySkill, onOpenSkillFile, initialPluginId, initialMarketplace, standalone=false, sessionIdle = false }: NativeIntegrationsProps) {
  ...
  const [initialMarketplaceAdd,setInitialMarketplaceAdd]=useState(Boolean(initialMarketplace?.add));
  // The child captures this entry request in its own initial state. Consume it
  // here so visiting MCP settings or changing owner cannot replay the dialog.
  useEffect(()=>{setInitialMarketplaceAdd(false);},[]);
  ...
  const catalogOwner=useRef(`${page}:${hostId}:${targetKey}`);
  useEffect(() => {
    ...
    const nextOwner=`${page}:${hostId}:${targetKey}`;
    if(catalogOwner.current!==nextOwner){catalogOwner.current=nextOwner;setSelected(initialPluginId??null);setLiveMcp(false);setPlugins(null);setMcp(null);setQuery("");}
    if(connected)void reload(); return () => { epoch.current++; };
  ...
  return <section className={`settings-page native-integrations ${standalone?"standalone-plugin-detail":""}`} aria-label={page === "plugins" ? "Plugins" : "MCP servers"}>
    {standalone&&<header className="plugin-detail-toolbar"><button onClick={onClose}>Plugins</button><Icon name="chevron"/><span>{plugin?.title||plugin?.name||(loading?'Loading…':'Plugin')}</span></header>}
...
function PluginPage({ catalog, selected, connected, saving, setSaving, setCatalog, setError, bridge, target, hostId, onSelect, standalone=false, loading=false, installedActions }: { standalone?:boolean;loading?:boolean;installedActions?(plugin:NativePlugin,disabled?:boolean):ReactNode; catalog: NativePluginCatalog | null; selected?: NativePlugin; connected: boolean; saving: boolean; setSaving: (v: boolean) => void; setCatalog: (v: NativePluginCatalog) => void; setError: (v: string | null) => void; bridge: DesktopBridge; target?: WorkspaceTarget; hostId: string; onSelect: (id: string) => void }) {
  const epochForMutations = useRef(0);
  const mutationPending = useRef(false);
  const listOpener = useRef<HTMLButtonElement | null>(null);
  useEffect(() => () => { epochForMutations.current++; mutationPending.current = false; }, [hostId, JSON.stringify(target), connected]);
  const selectedId = useRef(selected?.id); selectedId.current = selected?.id;
  const openWebsite = async (url:string) => { const ticket=epochForMutations.current,id=selected?.id; setError(null); try { await bridge.openExternal(url); } catch { if(ticket===epochForMutations.current&&id===selectedId.current)setError("The plugin website could not be opened."); } };
  const mutate = async (mutation: NativePluginMutation): Promise<boolean> => { if (!connected || saving || mutationPending.current) return false; mutationPending.current = true; const requestEpoch = epochForMutations.current; setSaving(true); setError(null); try { const value = await bridge.mutatePlugin(target, mutation, hostId); if (requestEpoch === epochForMutations.current) setCatalog(value); return requestEpoch === epochForMutations.current; } catch (cause) { if (requestEpoch === epochForMutations.current) setError(message(cause)); return false; } finally { if (requestEpoch === epochForMutations.current) { mutationPending.current = false; setSaving(false); } } };
  return <div className="integration-layout"><aside className={`integration-list ${selected||standalone ? "integration-list-hidden" : ""}`} aria-label="Installed plugins">{!catalog && <p role="status">{connected ? "Loading plugins…" : "Plugin catalog unavailable offline."}</p>}{catalog?.plugins.length === 0 && <p className="integration-placeholder">No plugins found in this native configuration.</p>}{catalog?.plugins.map(plugin => <div key={plugin.id} className="installed-plugin-row" data-enabled={plugin.enabled}>
    <button type="button" className="installed-plugin-open" aria-label={`Open ${plugin.title || plugin.name}`} onClick={event => { listOpener.current = event.currentTarget; onSelect(plugin.id); }}>
      <span className="installed-plugin-artwork" aria-hidden="true"><Icon name="skill"/></span>
      <span className="installed-plugin-label"><strong>{plugin.title || plugin.name}</strong><small>{plugin.description || `${plugin.version} · ${plugin.scope === "project" ? "This project" : "Personal"}`}{plugin.shadowed ? " · Shadowed" : ""}</small></span>
    </button>
    <NativeSwitch className="plugin-enabled-switch" label={`Enable ${plugin.title || plugin.name}`} checked={plugin.enabled} disabled={!connected || loading || saving || !plugin.canToggle} title={plugin.canToggle ? "Changes apply to new native sessions" : plugin.configurationReason || "Managed by native OMP configuration"} onChange={enabled => void mutate({expectedRevision:catalog!.revision,pluginId:plugin.id,operation:"enabled",enabled})}/>
  </div>)}</aside><main className="integration-detail">{selected ? <>{!standalone&&<button className="integration-back" type="button" onClick={() => { onSelect(""); requestAnimationFrame(() => { if (listOpener.current?.isConnected) listOpener.current.focus(); }); }}><Icon name="browserBack"/> Plugins</button>}<NativePluginSummary plugin={selected} onOpenWebsite={url=>void openWebsite(url)} actions={installedActions?.(selected,saving)} disabled={!connected || saving} onToggle={()=>void mutate({expectedRevision:catalog!.revision,pluginId:selected.id,operation:"enabled",enabled:!selected.enabled})}/>{(selected.features.length>0||selected.settings.length>0)&&<details className="plugin-native-configuration"><summary><Icon name="chevron"/><span>Configuration</span></summary><p className="integration-scope">Saved changes apply to new native sessions.</p>{selected.features.length > 0 && <section className="integration-section"><div className="integration-list-heading"><h3>Features</h3>{selected.canSetFeatures && <button className="native-reset" type="button" disabled={!connected || saving || selected.enabledFeatures === null} onClick={() => void mutate({expectedRevision:catalog!.revision,pluginId:selected.id,operation:"features",features:null})}>Use defaults</button>}</div>{selected.features.map(feature => <label className="integration-check" key={feature.name}><input type="checkbox" checked={(selected.enabledFeatures ?? selected.features.filter(item => item.default).map(item => item.name)).includes(feature.name)} disabled={!connected || saving || !selected.canSetFeatures} onChange={event => { const current = new Set(selected.enabledFeatures ?? selected.features.filter(item => item.default).map(item => item.name)); if (event.target.checked) current.add(feature.name); else current.delete(feature.name); void mutate({ expectedRevision: catalog!.revision, pluginId: selected.id, operation: "features", features: [...current] }); }}/><span><strong>{feature.name}</strong>{feature.description && <small>{feature.description}</small>}</span></label>)}</section>}{selected.settings.length > 0 && <section className="integration-section"><h3>Settings</h3>{!selected.canSetSettings && <p className="integration-note">{selected.configurationReason || "This plugin’s settings are read-only in the current native host."}</p>}{selected.settings.map(setting => <PluginSettingEditor key={`${selected.id}:${setting.key}`} plugin={selected} setting={setting} catalog={catalog!} connected={connected} saving={saving} mutate={mutate}/>)}</section>}</details>}</> : standalone?<p role="status">{loading?"Loading plugin…":connected?"This plugin is no longer available in this configuration.":"Plugin details are unavailable offline."}</p>:null}</main></div>;
}
