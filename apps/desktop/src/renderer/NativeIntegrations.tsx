...
export function NativeIntegrations({ bridge, hostId, hostName, connected, target, page, onClose, onPageChange, sessionIdle = false }: NativeIntegrationsProps) {
  ...
  const [marketplaces,setMarketplaces]=useState(false),[addMcp,setAddMcp]=useState(false);
  ...
  return <section className="settings-page native-integrations" aria-label={page === "plugins" ? "Plugins" : "MCP servers"}>
    ...
    <header className="integration-heading"><div><h1>Plugins</h1><p>Manage plugins and MCPs on {hostName}</p></div><div className="integration-header-actions"><div ref={setActionsRoot}/><button className="icon-button" aria-label="Reload integrations" title="Reload integrations" disabled={!connected || loading || saving} onClick={() => void reload()}><Icon name="refresh"/></button></div></header>
